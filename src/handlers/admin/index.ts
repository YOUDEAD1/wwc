/**
 * Admin dashboard â€” fully button-driven.
 *
...
async function autoFulfillPreordersAfterRestock(
  ...
  try {
    ...
    if (result.fulfilled > 0) {
      await ctx.reply(
        `âœ… Auto-delivered ${result.fulfilled} pending preorder(s) for product #${productId}.`,
      );
    }
  } catch (err) {
 ...
 * untouched so non-admin users never see the "â›” Admin only" reply
 * for ordinary chat messages.
 */
const requireAdmin: MiddlewareFn<AppCtx> = async (ctx, next) => {
  if (!ctx.from || !(await isAdmin(ctx.from.id))) {
    if (ctx.callbackQuery) {
      await ctx.answerCallbackQuery({ text: 'â›” Admin only.', show_alert: true });
    } else {
      await ctx.reply('â›” Admin only.');
    }
    return;
  }
  return next();
...
const PER_PAGE = 8;
const ROOT_TEXT = 'ðŸ›  *Admin Panel*\n\nTap a section to manage it.';

function rootMenu(): InlineKeyboard {
  return new InlineKeyboard()
    .text('ðŸ“¦ Products', 'adm:prod')
    .text('ðŸ—‚ Categories', 'adm:cat')
    .row()
    .text('ðŸ’³ Payment Methods', 'adm:pay')
    .text('ðŸ’° Top-Up Requests', 'adm:dep')
    .row()
    .text('ðŸ‘¥ Users', 'adm:usr:0')
    .text('ðŸ“£ Broadcast', 'adm:ann')
    .row()
    .text('ðŸŽ Referrals', 'adm:refs:0')
    .text('ðŸ§¾ Orders', 'adm:ord:0')
    .row()
    .text('ðŸŽ¨ Customize', 'adm:cust')
    .text('âš™ï¸ Bot Settings', 'adm:bot')
    .row()
    .text('ðŸ¤– AI Setup', 'adm:ai')
    .text('ðŸ“Š Stats', 'adm:stats')
    .row()
    .text('ðŸ”Œ API', 'adm:api')
    .text('ðŸ’¸ Promos', 'adm:promo')
    .row()
    .text('ðŸŽ Gift Codes', 'adm:gift')
    .row()
    .text('ðŸ’Ž Custom Prices', 'adm:price')
    .row()
    .text('ðŸ  Main Menu', 'adm:close');
}

const backRow = (kb: InlineKeyboard) => kb.row().text('â¬…ï¸ Back', 'adm:root');
...
adminBot.callbackQuery('adm:close', async (ctx) => {
  ...
  // Re-fire /start so the admin lands on the regular Main Menu â€” that's
  // the expected behaviour of the new "ðŸ  Main Menu" button on the
  // panel root, replacing the old "âŒ Close".
...
// reload settings shortcut. We deliberately keep this lean for now â€”
// each item edits a single key in the `settings` table.
adminBot.callbackQuery('adm:bot', async (ctx) => {
  await ctx.answerCallbackQuery();
  ctx.session.adminFlow = undefined;
  const kb = new InlineKeyboard()
    .text('ðŸ”— Set Channel URL', 'adm:cust:channel')
    .row()
    .text('ðŸ“„ Set Email PDF URL', 'adm:bot:emailpdf')
    .row()
    .text('ðŸ’¬ Set Admin Contact URL', 'adm:bot:contact')
    .row()
    .text('ðŸ“˜ Edit Bot Tutorial', 'adm:bot:tut')
    .row()
    .text('ðŸ” Reload Settings', 'adm:reload');
  backRow(kb);
...
// Bot Tutorial editor â€” surfaces the same fields as the legacy
...
async function showBotTutorialEditor(ctx: AppCtx): Promise<void> {
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ“ Set Text', 'adm:bot:tut:settxt')
    .text('ðŸ§¹ Clear Text', 'adm:bot:tut:clrtxt')
    .row()
    .text('ðŸŽž Set File', 'adm:bot:tut:setfile')
    .text('ðŸ§¹ Clear File', 'adm:bot:tut:clrfile')
    .row()
    .text('ðŸ”— Set URL', 'adm:bot:tut:seturl')
    .text('ðŸ§¹ Clear URL', 'adm:bot:tut:clrurl')
    .row();
  backRow(kb);
...
adminBot.callbackQuery('adm:bot:tut:settxt', async (ctx) => {
  ...
  await ctx.reply('ðŸ“ Send the *Bot Tutorial* text now.', { parse_mode: 'Markdown' });
});
...
adminBot.callbackQuery('adm:bot:tut:setfile', async (ctx) => {
  ...
  await ctx.reply(
    'ðŸŽž Send a photo, video, or document â€” it will be re-sent inside the Bot Tutorial.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery('adm:bot:tut:seturl', async (ctx) => {
  ...
  await ctx.reply('ðŸ”— Send the tutorial *URL* (`http://` or `https://`).', {
    parse_mode: 'Markdown',
  });
});
...
adminBot.callbackQuery('adm:ai', async (ctx) => {
  ...
  ctx.session.adminFlow = undefined;
  const kb = new InlineKeyboard()
    .text('ðŸ”‘ Set AI API Key', 'adm:ai:key')
    .row()
    .text('ðŸ’¬ Set AI Prompt', 'adm:ai:prompt');
  backRow(kb);
  await ctx.editMessageText(
    [
      'ðŸ¤– *AI Setup*',
      ...
      'â€¢ `AIzaâ€¦` â†’ Google AI Studio (Gemini)',
      'â€¢ `sk-â€¦`  â†’ OpenAI Chat Completions',
      '',
...
adminBot.callbackQuery('adm:ai:key', async (ctx) => {
  ...
  await ctx.editMessageText('ðŸ”‘ *Set AI API Key*\n\nSend the key (or `-` to clear).', {
    parse_mode: 'Markdown',
...
adminBot.callbackQuery('adm:gift', async (ctx) => {
  ...
  ctx.session.adminFlow = undefined;
  const kb = new InlineKeyboard()
    .text('âž• Create Code', 'adm:gift:add')
    .text('ðŸ“‹ List & Manage', 'adm:gift:list');
  backRow(kb);
  await ctx.editMessageText(
    'ðŸŽ *Gift Codes*\n\nIssue or manage one-time/limited gift codes that users can redeem from Settings â†’ Redeem Gift Code.',
    { parse_mode: 'Markdown', reply_markup: kb },
  );
});
...
adminBot.callbackQuery('adm:gift:add', async (ctx) => {
  ...
  // Use HTML so the (3â€“40 chars, Aâ€“Z 0â€“9 _ -) hint renders verbatim â€”
  ...
  await ctx.editMessageText(
    'ðŸŽ <b>Create Gift Code</b>\n\nSend the code â€” 3 to 40 chars (letters, digits, <code>_</code> or <code>-</code>). Or <code>/cancel</code>.',
...
async function showGiftCodeList(ctx: AppCtx): Promise<void> {
  ...
  for (const c of codes) {
    ...
    const exp = c.expires_at
      ? ` Â· exp ${new Date(c.expires_at).toISOString().slice(0, 10)}`
      : '';
    lines.push(`\`${c.code}\` Â· ${c.amount} USDT Â· used ${used}${cap}${exp}`);
    kb.text(`ðŸ—‘ ${c.code}`.slice(0, 60), `adm:gift:del:${c.code}`).row();
  }
  backRow(kb);
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
...
function statsRangeKeyboard(days?: number): InlineKeyboard {
  const kb = new InlineKeyboard();
  kb.text(days === 1 ? 'âœ… 24h' : '24h', 'adm:stats:r:1')
    .text(days === 7 ? 'âœ… 7d' : '7d', 'adm:stats:r:7')
    .text(days === 30 ? 'âœ… 30d' : '30d', 'adm:stats:r:30')
    .row()
    .text('ðŸ•’ Custom', 'adm:stats:custom')
    .text('ðŸ”„ Refresh', days ? `adm:stats:r:${days}` : 'adm:stats')
    .row()
    .text('â¬…ï¸ Back', 'adm:back');
  return kb;
}
...
async function showAdminStats(ctx: AppCtx, days?: number): Promise<void> {
  ...
  const lines: string[] = [];
  lines.push('ðŸ“Š *Bot Stats*');
  lines.push('');
  lines.push('ðŸŒ *All-Time Overview*');
  lines.push(`ðŸ‘¥ Users: *${allTime.users}*`);
  lines.push(`ðŸ“¦ Active products: *${allTime.active_products}*`);
  lines.push(`ðŸ—‚ Active categories: *${allTime.active_categories}*`);
  lines.push(`ðŸ§¾ Total orders: *${allTime.orders}*`);
  lines.push(`ðŸ’° Total revenue: *$${allTime.revenue.toFixed(2)}*`);
  lines.push(`ðŸ’³ Pending deposits: *${allTime.pending_deposits}*`);

  if (range) {
    const label = range.days === 1 ? 'Last 24 Hours' : `Last ${range.days} Days`;
    lines.push('');
    lines.push(`â± *${label}*`);
    lines.push(`ðŸ§¾ Orders: *${range.orders}*`);
    lines.push(`ðŸ“¦ Units sold: *${range.units}*`);
    lines.push(`ðŸ‘¤ Buyers: *${range.unique_buyers}*`);
    lines.push(`ðŸ’° Revenue: *$${range.revenue.toFixed(2)}*`);
    lines.push(`ðŸ’µ Approved topups: *${range.approved_deposits}* / *$${range.deposit_amount.toFixed(2)}*`);
    lines.push(`ðŸ†• New users: *${range.new_users}*`);
  }

  if (productSales.length > 0) {
    lines.push('');
    lines.push(days === 1 ? 'ðŸ† *24h Top Sellers*' : 'ðŸ† *Top Sellers*');
    productSales.slice(0, 5).forEach((row, i) => {
      const medal = ['ðŸ¥‡', 'ðŸ¥ˆ', 'ðŸ¥‰', '4.', '5.'][i] ?? `${i + 1}.`;
      lines.push(`${medal} ${escapeMd(row.product_name)} â€” *${row.units_sold}*u Â· *$${row.revenue.toFixed(2)}*`);
    });
    lines.push('');
    lines.push(days === 1 ? 'ðŸ“ˆ *24h Products Breakdown*' : 'ðŸ“ˆ *Products Breakdown*');
    productSales.slice(0, 30).forEach((row) => {
      const stock = row.stock_left !== null ? `stock *${row.stock_left}*` : '_deleted_';
      const last = row.last_sold_at ? ` Â· last *${row.last_sold_at.slice(0, 10)}*` : '';
      lines.push(`â€¢ ${escapeMd(row.product_name)}: *${row.units_sold}*u Â· *$${row.revenue.toFixed(2)}* Â· ${stock}${last}`);
    });
    if (productSales.length > 30) lines.push(`_â€¦and ${productSales.length - 30} more products_`);
  } else if (days) {
...
adminBot.callbackQuery('adm:stats', async (ctx) => {
  ...
  lines.push('');
  lines.push(`ðŸ‘¥ Users: *${s.users}*`);
  lines.push(`ðŸ“¦ Active products: *${s.active_products}*`);
  lines.push(`ðŸ—‚ Active categories: *${s.active_categories}*`);
  lines.push(`ðŸ§¾ Total orders: *${s.orders}*`);
  lines.push(`ðŸ’° Total revenue: *$${s.revenue.toFixed(2)}*`);
  lines.push(`ðŸ’³ Pending deposits: *${s.pending_deposits}*`);

  const visibleProductSales = productSales;

  if (visibleProductSales.length > 0) {
    lines.push('');
    lines.push('ðŸ† *Top Sellers (by revenue)*');
    visibleProductSales.slice(0, 5).forEach((r, i) => {
      const medal = ['ðŸ¥‡', 'ðŸ¥ˆ', 'ðŸ¥‰', '4.', '5.'][i] ?? `${i + 1}.`;
      lines.push(
        `${medal} ${escapeMd(r.product_name)} â€” *${r.units_sold}* units Â· *$${r.revenue.toFixed(
          2,
        )}*`,
      );
    });
  ...
  if (visibleProductSales.length > 0) {
    lines.push('');
    lines.push('ðŸ“ˆ *All Products â€” Sales Breakdown*');
    const cap = 30;
    visibleProductSales.slice(0, cap).forEach((r) => {
      const stockStr =
        r.stock_left !== null ? `stock *${r.stock_left}*` : '_deleted_';
      const lastStr = r.last_sold_at
        ? ` Â· last *${r.last_sold_at.slice(0, 10)}*`
        : '';
      lines.push(
        `â€¢ ${escapeMd(r.product_name)}: *${r.units_sold}*u Â· *$${r.revenue.toFixed(
          2,
        )}* Â· ${stockStr}${lastStr}`,
      );
    });
    if (visibleProductSales.length > cap) {
      lines.push(
        `_â€¦and ${visibleProductSales.length - cap} more (download PDF for full list)_`,
      );
    }
  }
...
function adminApiKeyboard(): InlineKeyboard {
  const kb = new InlineKeyboard()
    .text('ðŸ‘¥ API Users', 'adm:api:users:0')
    .text('ðŸ§¾ API Orders', 'adm:api:orders:0')
    .row()
    .copyText('ðŸ”— Copy Endpoint', apiBaseUrl())
    .row()
    .url('ðŸ“˜ Open API Docs', apiBaseUrl())
    .row()
    .text('ðŸ”„ Refresh', 'adm:api');
  backRow(kb);
  return kb;
}
...
async function showAdminApiOverview(ctx: AppCtx): Promise<void> {
  try {
    ...
    const lines: string[] = [
      'ðŸ”Œ *Reseller API Control Panel*',
      '',
      `ðŸŸ¢ Active keys: *${s.activeKeys}* / ${s.totalKeys}`,
      `ðŸ‘¥ API users: *${s.totalUsers}*`,
      `ðŸ§¾ API orders: *${s.totalOrders}*`,
      `ðŸ’° API spend: *${apiMoney(s.totalSpent)} USDT*`,
      '',
      'â± *Live Usage*',
      `24h: *${s.orders24h}* orders Â· *${apiMoney(s.spend24h)} USDT*`,
      `7d: *${s.orders7d}* orders Â· *${apiMoney(s.spend7d)} USDT*`,
      `30d: *${s.orders30d}* orders Â· *${apiMoney(s.spend30d)} USDT*`,
      '',
      '*Endpoint*',
      `\`${s.endpoint}\``,
    ];
    ...
    if (s.topUsers.length > 0) {
      lines.push('', 'ðŸ† *Top API Users*');
      s.topUsers.forEach((u, i) => {
        const marker = u.active ? 'ðŸŸ¢' : 'ðŸ”´';
        lines.push(
          `${i + 1}. ${marker} ${escapeMd(apiUserLabel(u))} â€” *${u.orders}* orders Â· *${apiMoney(u.totalSpent)}*`,
        );
      });
    }

    if (s.recentOrders.length > 0) {
      lines.push('', 'ðŸ§¾ *Recent API Orders*');
      s.recentOrders.forEach((o) => {
        const id = o.orderPublicId ?? `#${o.orderDbId}`;
        lines.push(
          `â€¢ \`${id}\` Â· ${escapeMd(o.productName)} Ã—${o.qty} Â· *${apiMoney(o.total)}*`,
        );
      });
    }
...
function apiUsersKeyboard(page: number, pages: number, rows: Array<{ userId: number; active: boolean }>): InlineKeyboard {
  ...
  for (const row of rows) {
    kb.text(`${row.active ? 'ðŸŸ¢' : 'ðŸ”´'} ${row.userId}`, `adm:api:user:${row.userId}`);
    kb.row();
  }
  if (pages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:api:users:${page - 1}`);
    kb.text(`${page + 1}/${pages}`, 'noop:api-users-page');
    if (page + 1 < pages) kb.text('Next â–¶ï¸', `adm:api:users:${page + 1}`);
    kb.row();
  }
  kb.text('ðŸ”„ Refresh', `adm:api:users:${page}`).text('ðŸ”Œ API Home', 'adm:api');
  backRow(kb);
  return kb;
}
  ...
  try {
    ...
    if (data.rows.length === 0) {
      ...
      data.rows.forEach((u, i) => {
        ...
        const status = u.active ? 'ðŸŸ¢ Active' : 'ðŸ”´ Disabled';
        lines.push(
          `${n}. *${escapeMd(apiUserLabel(u))}*`,
          `   ${status} Â· orders *${u.orders}* Â· spent *${apiMoney(u.totalSpent)} USDT*`,
          `   balance *${apiMoney(u.balance)} USDT* Â· last ${escapeMd(apiDate(u.lastUsedAt ?? u.lastOrderAt))}`,
        );
      });
    }
    await ctx.editMessageText(lines.join('\n').slice(0, 3900), {
...
function apiOrdersKeyboard(page: number, pages: number): InlineKeyboard {
  const kb = new InlineKeyboard();
  if (pages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:api:orders:${page - 1}`);
    kb.text(`${page + 1}/${pages}`, 'noop:api-orders-page');
    if (page + 1 < pages) kb.text('Next â–¶ï¸', `adm:api:orders:${page + 1}`);
    kb.row();
  }
  kb.text('ðŸ”„ Refresh', `adm:api:orders:${page}`).text('ðŸ”Œ API Home', 'adm:api');
  backRow(kb);
  return kb;
}
...
async function showAdminApiOrders(ctx: AppCtx, page: number): Promise<void> {
  try {
    ...
    const lines = ['ðŸ§¾ *Recent API Orders*', '', `Total: *${data.total}*`, ''];
    if (data.rows.length === 0) {
      ...
      data.rows.forEach((o, i) => {
        ...
        const id = o.orderPublicId ?? `#${o.orderDbId}`;
        lines.push(
          `${n}. \`${id}\` â€” *${escapeMd(o.productName)}*`,
          `   User: ${escapeMd(apiUserLabel(o))} Â· qty *${o.qty}* Â· *${apiMoney(o.total)} USDT*`,
          `   ${escapeMd(apiDate(o.createdAt))}${o.requestId ? ` Â· req \`${escapeMd(o.requestId.slice(0, 32))}\`` : ''}`,
        );
      });
    }
    await ctx.editMessageText(lines.join('\n').slice(0, 3900), {
...
function apiUserDetailKeyboard(userId: number, active: boolean): InlineKeyboard {
  const kb = new InlineKeyboard();
  if (active) {
    kb.text('âŒ Disable API Key', `adm:api:disable:${userId}`).row();
  }
  kb.text('ðŸ‘¥ API Users', 'adm:api:users:0').text('ðŸ”„ Refresh', `adm:api:user:${userId}`).row();
  kb.text('ðŸ”Œ API Home', 'adm:api');
  backRow(kb);
  return kb;
}
...
async function showAdminApiUser(ctx: AppCtx, userId: number): Promise<void> {
  try {
    ...
    if (!data.user) {
      await ctx.editMessageText('âŒ API user not found.', {
        reply_markup: backRow(new InlineKeyboard()),
      });
      return;
    }
    ...
    const lines = [
      ...
      `Status: *${u.active ? 'ðŸŸ¢ Active' : 'ðŸ”´ Disabled'}*`,
      `Key: \`${u.keyPrefix ? `${u.keyPrefix}â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢` : 'â€”'}\``,
    ...
    if (data.recentOrders.length > 0) {
      lines.push('', 'ðŸ§¾ *Last Orders*');
      data.recentOrders.forEach((o) => {
        const id = o.orderPublicId ?? `#${o.orderDbId}`;
        lines.push(`â€¢ \`${id}\` Â· ${escapeMd(o.productName)} Ã—${o.qty} Â· *${apiMoney(o.total)}*`);
      });
    }
    await ctx.editMessageText(lines.join('\n').slice(0, 3900), {
      parse_mode: 'Markdown',
    ...
    if (data.rows.length === 0) {
      ...
      data.rows.forEach((s, i) => {
        ...
        lines.push(
          ...
          `   ${status} Â· balance ${s.last_balance === null ? 'â€”' : `*${apiMoney(Number(s.last_balance))}*`} Â· last ${escapeMd(apiDate(s.last_sync_at))}`,
...
async function showSupplierDetail(ctx: AppCtx, id: number): Promise<void> {
  try {
    ...
    if (!source) {
      await supplierSendOrEdit(ctx, 'âŒ Supplier not found.', {
        reply_markup: backRow(new InlineKeyboard()),
      });
      return;
    }
    ...
    const lines = [
      ...
      `Balance: ${source.last_balance === null ? 'â€”' : `*${apiMoney(Number(source.last_balance))}*`}`,
    ...
    if (links.length > 0) {
      for (const link of links.slice(0, 8)) {
        ...
        lines.push(
          `â€¢ #${link.local_product_id} ${escapeMd(product?.name ?? 'unknown')} -> \`${escapeMd(link.supplier_product_id)}\``,
          `  auto ${link.auto_order ? 'ON' : 'OFF'} Â· sync ${link.auto_sync_stock ? 'ON' : 'OFF'} Â· stock ${link.supplier_stock ?? 'â€”'} Â· cost ${link.supplier_cost ?? 'â€”'}`,
        );
      }
    }
    if (logs.length > 0) {
      ...
      logs.forEach((l) => {
        lines.push(
          `â€¢ ${escapeMd(apiDate(l.created_at))} Â· ${escapeMd(l.status)} Â· product #${l.local_product_id ?? 'â€”'}`,
        );
      });
    }
    await supplierSendOrEdit(ctx, lines.filter(Boolean).join('\n').slice(0, 3900), {
...
async function showSupplierCatalog(
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const label = `${status} ${row.product.name} Â· ${cost}->${sell} Â· ${supplierStockLabel(row.product)}`.slice(0, 60);
...
async function showSupplierCatalogProduct(
  ...
  try {
    ...
    if (!product) {
      await supplierSendOrEdit(ctx, 'âŒ Supplier product not found. Refresh the supplier catalog.', {
        reply_markup: backRow(new InlineKeyboard()),
      });
      return;
    }
...
adminBot.callbackQuery(/^adm:api:supplier:test:(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const sample = result.sampleProducts
      .map((p) => `â€¢ \`${escapeMd(p.id)}\` ${escapeMd(p.name)} Â· ${p.price ?? 'â€”'} Â· stock ${p.stock ?? 'â€”'}`)
      .join('\n');
    await ctx.reply(
      [
        result.ok ? 'âœ… *Supplier Test OK*' : 'âš ï¸ *Supplier Test Partial/Failed*',
        '',
        `Balance: ${result.balance === null ? 'â€”' : `*${apiMoney(result.balance)}*`}`,
...
adminBot.callbackQuery(/^adm:api:supplier:logs:(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    const lines = ['ðŸ§¾ *Supplier Order Logs*', '', `Supplier ID: \`${id}\``, ''];
    if (logs.length === 0) {
      lines.push('_No supplier order attempts yet._');
    } else {
      logs.forEach((l, i) => {
        lines.push(
          `${i + 1}. *${escapeMd(l.status)}* Â· ${escapeMd(apiDate(l.created_at))}`,
          `   Local order: \`${l.local_order_id ?? 'â€”'}\` Â· product: \`${l.local_product_id ?? 'â€”'}\``,
...
adminBot.callbackQuery(/^adm:api:supplier:delete:(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    await ctx.reply(`âœ… Supplier #${id} deleted.`);
...
// after `escapeHtml`:
//
//   1. Global feed             â€” `adm:ord:<page>`
//      Newest paid orders across the whole shop. Each row exposes a
//      "View" button that opens the per-order detail screen.
//   2. Per-user orders         â€” `adm:ord:u:<telegram_id>:<page>`
//      Reached from the user card "ðŸ§¾ View Orders" button. Same row
//      layout as the global feed, scoped to one buyer.
//   3. Per-product orders      â€” `adm:ord:p:<product_id>:<page>`
//      Reached from the product editor "ðŸ§¾ View Buyers" button. Same
//      row layout, scoped to one product.
//
//   4. Per-order detail        â€” `adm:ord:v:<id>`
...
async function showOrdersList(
  ...
  if (scope.kind === 'all') {
    ...
    rows = r.rows;
    total = r.total;
    header = `ðŸ§¾ <b>Buyers of product #${scope.product_id}</b>`;
    pagePrefix = `adm:ord:p:${scope.product_id}`;
  }
  const totalPages = Math.max(1, Math.ceil(total / perPage));
  const page = scope.kind === 'all' ? scope.page : scope.page;
  if (rows.length === 0) {
    const kb = new InlineKeyboard().text('â¬…ï¸ Back', backCb);
  ...
  const lines = [`${header} â€” page ${page + 1}/${totalPages}  (total ${total})`, ''];
  const kb = new InlineKeyboard();
  for (const o of rows) {
    ...
    const statusEmoji =
      o.status === 'paid' ? 'âœ…' : o.status === 'refunded' ? 'â†©ï¸' : 'âœ–ï¸';
    lines.push(
      `${statusEmoji} <code>#${o.id}</code> ${safeName} Ã— ${o.qty} â€” ` +
        `<b>$${Number(o.total).toFixed(2)}</b> Â· ${safeHandle} Â· ${date}`,
    );
    // Compact one-tap row: jump straight into the order detail.
    kb.text(
      `ðŸ”Ž #${o.id} ${o.product_name} Ã— ${o.qty}`.slice(0, 60),
      `adm:ord:v:${o.id}`,
    ).row();
  }
  if (page > 0) kb.text('â—€ï¸ Prev', `${pagePrefix}:${page - 1}`);
  if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `${pagePrefix}:${page + 1}`);
  kb.row().text('â¬…ï¸ Back', backCb);
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'HTML',
    reply_markup: kb,
  });
...
adminBot.callbackQuery(/^adm:ord:v:(\d+)$/, async (ctx) => {
  ...
  if (!order) {
    await ctx.editMessageText('âš ï¸ Order not found.', {
      reply_markup: backRow(new InlineKeyboard()),
    });
    return;
  }
  ...
  const lines = [
    ...
    Number(order.discount) > 0
      ? `<b>Discount:</b> âˆ’$${Number(order.discount).toFixed(2)}` +
  ...
  const kb = new InlineKeyboard();
  if (buyer) {
    kb.text('ðŸ‘¤ Open Buyer', `adm:usr:v:${order.user_id}`).row();
  }
  if (order.product_id !== null) {
    kb.text('ðŸ§¾ More buyers of this product', `adm:ord:p:${order.product_id}:0`).row();
  }
  kb.text('â¬…ï¸ Back to orders', 'adm:ord:0').text('ðŸ  Main', 'adm:root');
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'HTML',
    reply_markup: kb,
  });
...
adminBot.callbackQuery('adm:reload', async (ctx) => {
  ...
  cache.clearAll();
  await ctx.answerCallbackQuery({ text: 'ðŸ” Settings reloaded.' });
  await showRoot(ctx);
});

adminBot.callbackQuery('adm:clr', async (ctx) => {
  cache.clearAll();
  await ctx.answerCallbackQuery({ text: 'ðŸ§¹ Cache cleared.' });
  await showRoot(ctx);
});
...
adminBot.callbackQuery('adm:cat', async (ctx) => {
  ...
  ctx.session.adminFlow = undefined;
  const kb = new InlineKeyboard()
    .text('âž• Add Category', 'adm:cat:add')
    .text('ðŸ“‹ List & Manage', 'adm:cat:list');
  backRow(kb);
  await ctx.editMessageText('ðŸ—‚ *Categories*', { parse_mode: 'Markdown', reply_markup: kb });
});
...
async function showCategoryList(ctx: AppCtx): Promise<void> {
  ...
  const lines = ['ðŸ—‚ *Categories*', ''];
  const kb = new InlineKeyboard();
  for (const c of cats) {
    lines.push(`#${c.id}  ${c.emoji ?? 'ðŸ“'} ${c.name}${c.active ? '' : '  _(hidden)_'}`);
    kb.text(`ðŸ—‘ #${c.id} ${c.name}`.slice(0, 60), `adm:cat:del:${c.id}`);
...
adminBot.callbackQuery('adm:prod', async (ctx) => {
  ...
  ctx.session.adminFlow = undefined;
  const kb = new InlineKeyboard()
    .text('âž• Add Product', 'adm:prod:add')
    .text('ðŸ“‹ List & Manage', 'adm:prod:list:0');
  backRow(kb);
  await ctx.editMessageText('ðŸ“¦ *Products*', { parse_mode: 'Markdown', reply_markup: kb });
});
...
adminBot.callbackQuery('adm:prod:add', async (ctx) => {
  ...
  const cats = await listAllCategories();
  if (cats.length === 0) {
    await ctx.editMessageText('âš ï¸ No categories yet. Add a category first.', {
      reply_markup: backRow(new InlineKeyboard()),
    });
    return;
  }
  const kb = new InlineKeyboard();
  cats.forEach((c, i) => {
    kb.text(`${c.emoji ?? 'ðŸ“'} ${c.name}`, `adm:prod:add:cat:${c.id}`);
    // Apply the per-category Bot API 9.4 button style configured via
    // /admin â†’ Customize â†’ Set Color â†’ ðŸ“‚ Product Categories. Falls
  ...
  backRow(kb);
  await ctx.editMessageText('ðŸ“¦ *Add Product*\n\nPick a category:', {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
});
...
async function showProductList(ctx: AppCtx, page: number): Promise<void> {
  ...
  const lines = [
    `ðŸ“¦ *Products* â€” page ${page + 1}/${totalPages}`,
    '',
    '_Tap â†‘ / â†“ to nudge one slot, â« / â¬ to jump straight to the top / bottom._',
    '_ðŸ“Œ = pinned (stays put even when out of stock). ðŸ’¤ = auto-moved here because it ran out of stock; will pop back to its old spot on restock._',
    '',
  ];
  const kb = new InlineKeyboard();
  for (const p of rows) {
    const flag = p.active ? 'ðŸŸ¢' : 'âšªï¸';
    const pinTag = p.is_pinned ? ' ðŸ“Œ' : '';
    const oosTag = p.stashed_sort_order !== null ? ' ðŸ’¤' : '';
    lines.push(
      `${flag} #${p.id}${pinTag}${oosTag}  ${p.name} â€” $${p.price}  (stock ${p.stock})`,
    );
    kb.text(`â†‘ #${p.id}`, `adm:prod:up:${p.id}:${page}`)
      .text(`â†“ #${p.id}`, `adm:prod:dn:${p.id}:${page}`)
      .text(`âœï¸ Edit #${p.id}`, `adm:prod:edit:${p.id}:${page}`)
      .row();
    // One-tap jump-to-end buttons â€” the bot-owner asked for an
    // easier reorder UX than "tap â†‘ thirty times" so each row gets
    // â« Top / â¬ Bottom alongside the nudge buttons. The pin toggle
    // sits next to them since pin status is the other thing that
    // affects ordering at a glance.
    kb.text(`â« Top #${p.id}`, `adm:prod:top:${p.id}:${page}`)
      .text(`â¬ Bottom #${p.id}`, `adm:prod:bot:${p.id}:${page}`)
      .text(
        p.is_pinned ? `ðŸ“Œ Pinned #${p.id}` : `ðŸ“Œ Pin #${p.id}`,
        `adm:prod:pin:${p.id}:${page}`,
      )
      .row();
    kb.text(p.active ? `ðŸ‘ Hide #${p.id}` : `ðŸ‘ Show #${p.id}`, `adm:prod:tog:${p.id}:${page}`)
      .text(`ðŸ†” ID #${p.id}`, `adm:prod:id:set:${p.id}:${page}`)
      .text(`ðŸ—‘ #${p.id}`, `adm:prod:del:${p.id}:${page}`)
      .row();
    // Per-product Add Items / Edit Pool shortcuts. Both reuse the
    // existing handlers â€” `ðŸ“¦ Add Items #N` arms the bulk-paste flow
    // for that product, `ðŸ”Ž Edit Pool #N` opens the Stock Items
    ...
    kb.text(`ðŸ“¦ Add Items #${p.id}`, `adm:prod:items:add:${p.id}:${page}`)
      .text(`ðŸ”Ž Edit Pool #${p.id}`, `adm:prod:items:view:${p.id}:${page}:0`)
      .row();
  }
  if (page > 0) kb.text('â—€ï¸ Prev', `adm:prod:list:${page - 1}`);
  if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:prod:list:${page + 1}`);
  backRow(kb);
  await ctx.editMessageText(lines.join('\n'), { parse_mode: 'Markdown', reply_markup: kb });
}
...
// page boundaries â€” swapping with a row on a different page just
...
adminBot.callbackQuery(/^adm:prod:(up|dn):(\d+):(\d+)$/, async (ctx) => {
  ...
  // OOS bottom-of-the-list value. Otherwise an â†‘ nudge would swap
  ...
  cache.del('cats');
  await ctx.answerCallbackQuery({
    text: direction === 'up' ? 'â†‘ Moved up' : 'â†“ Moved down',
  });
  await showProductList(ctx, page);
});

// One-tap jump-to-the-top / jump-to-the-bottom for the bot-owner's
// "make reorder easier" request. Both clear `stashed_sort_order` â€”
...
adminBot.callbackQuery(/^adm:prod:top:(\d+):(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    cache.del('cats');
    await ctx.answerCallbackQuery({ text: 'â« Moved to top' });
  } catch (err) {
...
adminBot.callbackQuery(/^adm:prod:bot:(\d+):(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    cache.del('cats');
    await ctx.answerCallbackQuery({ text: 'â¬ Moved to bottom' });
  } catch (err) {
...
adminBot.callbackQuery(/^adm:prod:pin:(\d+):(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    cache.del('cats');
    await ctx.answerCallbackQuery({
      text: nowPinned ? 'ðŸ“Œ Pinned in place' : 'ðŸ”“ Unpinned',
    });
  } catch (err) {
...
//
// Surfaced from the product list as `âœï¸ Edit #N`. Renders a card
...
// applies the patch â€” e.g. tap "Set Premium Emoji", send a ðŸŽ¬ premium
...
async function showProductEditor(
  ...
  // swallowed there â€” if Supabase rejected the update for any reason
  ...
  if (!p) {
    await ctx.editMessageText('âš ï¸ Product not found.', {
      reply_markup: backRow(new InlineKeyboard()),
    });
    return;
  }
  ...
  // product isn't unlimited â€” the products.stock column is just a
  // denormalised mirror, the truth is `countAvailableProductItems()`.
  // This keeps the card honest even if a sync ever misses.
  const stockCell = p.unlimited_stock ? 'âˆž' : supplierLink ? String(p.stock) : String(itemsCount);
  // Per-product custom-price override count â€” surfaced inline + drives
  ...
  // now falls back to a single default `Details` prompt â€” surface
  ...
  // tap ðŸ—‚ Fields to override with a typed multi-field spec.
  const deliveryStateLabel = p.delivery_form_enabled
    ? deliveryFieldsCount > 0
      ...
      : '*ON* (using default `Details` prompt â€” tap ðŸ—‚ Fields to customise)'
    : '_OFF_';
  ...
  const referralLabel =
    p.referral_required_count > 0
      ...
      : '_OFF_';
  const lines = [
    `âœï¸ *Edit Product #${p.id}*`,
    '',
    `*Name:* ${p.name}`,
    `*Price:* ${Number(p.price).toFixed(2)} USDT`,
    `*Stock:* ${stockCell}`,
    supplierLink
      ? `*Supplier link:* \`${supplierLink.supplier_product_id}\` Â· stock sync *${supplierLink.auto_sync_stock ? 'ON' : 'OFF'}*`
      : null,
  ...
  const kb = new InlineKeyboard();
  kb.text('ðŸŽ¬ Set Premium Emoji', `adm:prod:emoji:set:${p.id}:${page}`)
    .text('ðŸ§¹ Clear Emoji', `adm:prod:emoji:clr:${p.id}:${page}`)
    .row();
  kb.text('ðŸ“ Set Note Text', `adm:prod:note:settxt:${p.id}:${page}`)
    .text('ðŸ§¹ Clear Note', `adm:prod:note:clr:${p.id}:${page}`)
    .row();
  kb.text('ðŸ“„ Edit Description', `adm:prod:desc:set:${p.id}:${page}`)
    .text('ðŸ§¹ Clear Desc', `adm:prod:desc:clr:${p.id}:${page}`)
    .row();
  kb.text('â­ Edit Warranty', `adm:prod:war:set:${p.id}:${page}`)
    .text('ðŸ§¹ Clear Warranty', `adm:prod:war:clr:${p.id}:${page}`)
    .row();
  kb.text('ðŸ“˜ Tutorial Text', `adm:prod:tut:settxt:${p.id}:${page}`)
    .text('ðŸŽž Tutorial File', `adm:prod:tut:setfile:${p.id}:${page}`)
    .row();
  ...
  kb.text('ðŸ§¹ Clear File', `adm:prod:tut:clrfile:${p.id}:${page}`)
    .text('ðŸ”— Tutorial URL', `adm:prod:tut:seturl:${p.id}:${page}`)
    .row();
  kb.text('ðŸ§¹ Clear Tutorial', `adm:prod:tut:clr:${p.id}:${page}`).row();
  kb.text(`ðŸ“¦ Add Items (pool: ${itemsCount})`, `adm:prod:items:add:${p.id}:${page}`)
    .text('ðŸ§¹ Clear Pool', `adm:prod:items:clr:${p.id}:${page}`)
    .row();
  // Stock Inspection â€” bot-owner asked for a way to audit remaining
  ...
  kb.text(
    `ðŸ”Ž View Stock Items (${itemsCount})`,
    `adm:prod:items:view:${p.id}:${page}:0`,
  ).row();
  kb.text(
    p.unlimited_stock ? 'â™¾ Unlimited: ON' : 'â™¾ Unlimited: OFF',
    `adm:prod:unl:tog:${p.id}:${page}`,
  )
    .text(
      p.is_pinned ? 'ðŸ“Œ Pinned: ON' : 'ðŸ“Œ Pinned: OFF',
      `adm:prod:pin:${p.id}:${page}`,
    )
    .row();
  kb.text('ðŸ’° Edit Price', `adm:prod:price:set:${p.id}:${page}`)
    .text('ðŸ”¢ Edit Stock', `adm:prod:stock:set:${p.id}:${page}`)
    .text('ðŸ…°ï¸ Edit Name', `adm:prod:name:set:${p.id}:${page}`)
    .row();
  kb.text('ðŸŽ Referral Pay', `adm:prod:ref:set:${p.id}:${page}`)
    .text('ðŸ§¹ Disable Referral Pay', `adm:prod:ref:clr:${p.id}:${page}`)
    .row();
  ...
  // when nobody has an override on this product â€” the disabled-style
  // button would just be visual noise.
  if (overrideCount > 0) {
    kb.text(
      `ðŸ§¹ Clear all custom prices (${overrideCount})`,
      `adm:prod:cpclr:${p.id}:${page}`,
    ).row();
  }
  kb.text('ðŸ†” Edit ID', `adm:prod:id:set:${p.id}:${page}`)
    .text('ðŸ”— Share Link', `adm:prod:share:${p.id}:${page}`)
    .row();
  ...
  kb.text(
    p.delivery_form_enabled ? 'ðŸ“¥ Delivery Form: ON' : 'ðŸ“¥ Delivery Form: OFF',
    `adm:prod:del:tog:${p.id}:${page}`,
  ).row();
  if (p.delivery_form_enabled) {
    kb.text('âœï¸ Instruction', `adm:prod:del:instr:${p.id}:${page}`)
      .text('ðŸ—‚ Fields', `adm:prod:del:fields:${p.id}:${page}`)
      .row();
    kb.text('âœ… Success Message', `adm:prod:del:succ:${p.id}:${page}`)
      .text('ðŸ¤ Vendor Chat ID', `adm:prod:del:vendor:${p.id}:${page}`)
      .row();
    kb.text('ðŸ· Vendor Label', `adm:prod:del:vlabel:${p.id}:${page}`)
      .text('ðŸ§¹ Clear Delivery', `adm:prod:del:clr:${p.id}:${page}`)
      .row();
  }
  kb.text('ðŸ§¾ View Buyers', `adm:ord:p:${p.id}:0`).row();
  kb.text('â¬…ï¸ Back to list', `adm:prod:list:${page}`);
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
...
adminBot.callbackQuery(/^adm:prod:emoji:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸŽ¬ Send a single *premium* emoji as your next message â€” the bot will read its `custom_emoji_id` and save it.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:note:settxt:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸ“ Send the *View Note* text now (any premium emojis you include are preserved). Send `/cancel` to abort.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:desc:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸ“„ Send the new *description* text now. Send `/cancel` to abort.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:war:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'â­ Send the new *warranty* text now. Send `/cancel` to abort.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:ref:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸŽ Send the referrals required to buy this product with Referral Pay (0 = disabled). Send `/cancel` to abort.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:tut:settxt:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸ“˜ Send the *Using Method* tutorial text. Premium emojis preserved. Send `/cancel` to abort.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:tut:setfile:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸŽž Send a photo, video, or document as your next message â€” it becomes the tutorial attachment.',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:tut:seturl:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    'ðŸ”— Send the tutorial *URL* as your next message (must start with `http://` or `https://`).',
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:del:tog:(\d+):(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    if (!p) {
      await ctx.reply('âš ï¸ Product not found.');
      return;
    }
    ...
    await ctx.reply(
      [
        'âš ï¸ Failed to toggle the Delivery Form.',
        '',
        `Error: \`${msg}\``,
...
adminBot.callbackQuery(/^adm:prod:del:instr:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    [
      'âœï¸ *Send the delivery-form instruction text now.*',
      '',
...
adminBot.callbackQuery(/^adm:prod:del:succ:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    [
      'âœ… *Send the success-card text now.*',
      '',
      'Shown after the buyer submits their details (e.g. _"Your details has been submitted successfully â€” our team will approve it shortly."_). Premium emojis preserved.',
      '',
...
adminBot.callbackQuery(/^adm:prod:del:fields:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    [
      'ðŸ—‚ *Send the field spec â€” one field per line.*',
      '',
...
adminBot.callbackQuery(/^adm:prod:del:vlabel:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    [
      'ðŸ· *Send the vendor display label.*',
      '',
      'Optional short string shown in the admin-facing summary (e.g. `@john_vendor` or `Workspace A`). Buyer never sees this â€” it\'s just to help you tell vendors apart in the editor.',
      '',
...
adminBot.callbackQuery(/^adm:prod:del:clr:(\d+):(\d+)$/, async (ctx) => {
  // Ack first â€” same reasoning as `adm:prod:del:tog` above.
  ...
  try {
    ...
    await ctx.reply(
      `âš ï¸ Failed to clear delivery config: \`${(err as Error)?.message ?? 'unknown error'}\``,
      { parse_mode: 'Markdown' },
    );
  }
});
...
// committing, allow accumulating large batches across messages â€” paste
...
// cleanly. We do NOT deduplicate against the existing pool here â€”
...
// admin pasting an entire 50 MB credentials dump into the chat â€” at
...
// is ~30 k payload lines â€” well above the staging cap above and
 ...
 * (text message, .txt upload, Clear tap) so the chat stays clean â€”
...
async function renderItemsStagingCard(
  ...
  const lines: string[] = [
    'ðŸ“¥ *Bulk-add to items pool â€” staging*',
    '',
    productLine,
    ...
    '_file â€” every message appends to the buffer above._',
    '',
    'Tap *âœ… Confirm & Add* to flush the buffer to the pool, *ðŸ§¹ Clear*',
    'to drop the staged lines and start over, or *âŒ Cancel* to exit',
  ...
  const kb = new InlineKeyboard()
    .text(`âœ… Confirm & Add (${staged.length})`, `adm:prod:items:confirm:${product_id}:${page}`)
    .row()
    .text('ðŸ§¹ Clear staged', `adm:prod:items:clear_stage:${product_id}:${page}`)
    .text('âŒ Cancel', `adm:prod:items:cancel:${product_id}:${page}`);
  ...
  // with a 400 â€” fall back to a fresh post in that case.
...
adminBot.callbackQuery(/^adm:prod:items:add:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    [
      'ðŸ“¦ *Bulk-add deliverables â€” paste, forward, or upload.*',
      '',
      'Send the deliverables across one or more messages â€” *one payload per line*.',
      ...
      '```',
      '',
      'ðŸ“Ž You can also upload a `.txt` file (one payload per line) and the',
      'bot will auto-parse it.',
      '',
      'Nothing is added to the pool until you tap *âœ… Confirm & Add* on the',
...
adminBot.callbackQuery(/^adm:prod:items:confirm:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({ text: `Adding ${staged.length} item(s)â€¦` });
  try {
    ...
    await ctx.reply('âŒ Could not save items â€” see logs for details.');
    return;
  }
  ctx.session.adminFlow = undefined;
  await ctx.reply(`âœ… Added \`${staged.length}\` item(s) to the pool.`, {
    parse_mode: 'Markdown',
  });
...
adminBot.callbackQuery(/^adm:prod:items:cancel:(\d+):(\d+)$/, async (ctx) => {
  const product_id = Number(ctx.match[1]);
  const page = Number(ctx.match[2]);
  await ctx.answerCallbackQuery({ text: 'Cancelled â€” nothing was added.' });
  ctx.session.adminFlow = undefined;
  await showProductEditor(ctx, product_id, page);
});
...
//   adm:prod:cpclr:<pid>:<page>     â†’ show confirm dialog
//   adm:prod:cpclr:ok:<pid>:<page>  â†’ run the delete
adminBot.callbackQuery(/^adm:prod:cpclr:(\d+):(\d+)$/, async (ctx) => {
  ...
  if (!p) {
    await ctx.editMessageText('âš ï¸ Product not found.', {
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', `adm:prod:list:${page}`),
    });
    return;
  }
  if (overrideCount === 0) {
    // Edge case â€” the count changed to 0 between the editor render
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ§¹ Yes, clear all', `adm:prod:cpclr:ok:${id}:${page}`)
    .text('âŒ Cancel', `adm:prod:edit:${id}:${page}`);
  await ctx.editMessageText(
    [
      `ðŸ§¹ *Clear all custom prices for #${p.id} ${escapeHtml(p.name)}?*`,
      '',
      ...
      '',
      'This cannot be undone â€” re-enter each user override manually if you change your mind.',
    ].join('\n'),
    { parse_mode: 'Markdown', reply_markup: kb },
  );
});
...
adminBot.callbackQuery(/^adm:prod:cpclr:ok:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({
    text: n === 0 ? 'No overrides to clear.' : `ðŸ§¹ Cleared ${n} override${n === 1 ? '' : 's'}.`,
  });
  await showProductEditor(ctx, id, page);
});
...
async function showStockInspectionPage(
  ...
  const lines: string[] = [
    `ðŸ”Ž *Stock Inspection â€” ${product.name}*`,
    '',
  ...
  if (pageCount > 1) {
    ...
    kb.text('ðŸ”„', `adm:prod:items:view:${product_id}:${productPage}:${safePage}`);
  ...
  kb.text('â¬…ï¸ Back', `adm:prod:edit:${product_id}:${productPage}`);
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
...
adminBot.callbackQuery(/^adm:prod:unl:tog:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({
    text: !p.unlimited_stock ? 'â™¾ Unlimited ON' : 'Unlimited OFF',
  });
  await showProductEditor(ctx, id, Number(ctx.match[2]));
});
...
adminBot.callbackQuery(/^adm:prod:price:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply('ðŸ’° Send the new *price* (number, e.g. `9.99`).', { parse_mode: 'Markdown' });
});
...
adminBot.callbackQuery(/^adm:prod:stock:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply('ðŸ”¢ Send the new *stock* (integer â‰¥ 0).', { parse_mode: 'Markdown' });
});
...
adminBot.callbackQuery(/^adm:prod:name:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply('ðŸ…°ï¸ Send the new product *name*.', { parse_mode: 'Markdown' });
});
...
adminBot.callbackQuery(/^adm:prod:id:set:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `ðŸ†” Current ID: \`${product_id}\`\nSend the new *product ID* (integer). âš ï¸ Make sure the new ID doesn't already exist.`,
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:prod:share:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `ðŸ”— *Share link for ${escapeMd(p.name)}*\n\n` +
      `\`${shareUrl}\`\n\n` +
...
adminBot.callbackQuery('adm:pay', async (ctx) => {
  ...
  ctx.session.adminFlow = undefined;
  const kb = new InlineKeyboard()
    .text('âž• Manual Method', 'adm:pay:add')
    .text('ðŸ“‹ List & Manage', 'adm:pay:list')
    .row()
    .text('ðŸŸ¢ Add USDT (TRC20)', 'adm:pay:add:usdt_trc20')
    .text('ðŸŸ¡ Add USDT (BEP20)', 'adm:pay:add:usdt_bep20')
    .row()
    .text('ðŸ”µ Add USDT (TON)', 'adm:pay:add:usdt_ton')
    .text('âšª Add LTC', 'adm:pay:add:ltc')
    .row()
    .text('ðŸŸ¡ Add Binance Pay', 'adm:pay:add:binance_pay')
  ...
  await ctx.editMessageText(
    [
      'ðŸ’³ *Payment Methods*',
      '',
      '*Manual* â€” name, instructions, min amount. Users submit a deposit request you approve from the *Deposits* tab.',
      '',
      '*Auto-verify* â€” pick a provider, set the wallet address, and the bot verifies the user\'s on-chain tx hash and credits the wallet automatically.',
    ].join('\n'),
    { parse_mode: 'Markdown', reply_markup: kb },
  );
});
...
> = {
  usdt_trc20: {
    title: 'ðŸŸ¢ *Add USDT (TRC20)*',
    namePlaceholder: 'USDT (TRC20)',
    addressPrompt:
      'Send the *TRON wallet address* (starts with `Tâ€¦`, 34 chars) that USDT TRC20 deposits should land in.',
  },
  usdt_bep20: {
    title: 'ðŸŸ¡ *Add USDT (BEP20)*',
    namePlaceholder: 'USDT (BEP20)',
    addressPrompt:
      'Send the *BSC wallet address* (starts with `0xâ€¦`, 42 chars) that USDT BEP20 deposits should land in.',
  },
  usdt_ton: {
    title: 'ðŸ”µ *Add USDT (TON)*',
    namePlaceholder: 'USDT (TON)',
    addressPrompt:
      'Send the *TON wallet address* (`EQâ€¦` or `UQâ€¦`, 48 chars) that USDT (TON Jetton) deposits should land in.',
  },
  ltc: {
    title: 'âšª *Add LTC (Litecoin)*',
    namePlaceholder: 'LTC',
    addressPrompt:
      'Send the *Litecoin address* (`Lâ€¦` / `Mâ€¦` / `ltc1â€¦`) that LTC deposits should land in.',
  },
};
...
async function showPaymentList(ctx: AppCtx): Promise<void> {
  ...
  const lines = ['ðŸ’³ *Payment Methods*', ''];
  const kb = new InlineKeyboard();
  for (const m of methods) {
            ...
            : m.provider === 'usdt_ton'
              ...
              : m.provider === 'ltc'
                ...
                : m.provider === 'bybit_pay'
                  ? 'auto â€¢ Bybit Pay'
                : 'auto â€¢ Binance Pay';
    lines.push(`#${m.id}  ${m.name} â€” _${tag}_`);
    if (m.address) {
      const addrLabel =
    ...
    // Per-method chrome controls. The button row reads "ðŸŽ¨ Color"
    // (cycles through none/blue/green/red/yellow) and "ðŸŒŸ Icon"
    // (waits for the admin's next emoji message â€” supports premium
    ...
    const iconTag = m.emoji_id ? 'â­' : (m.emoji_unicode ?? 'default');
    kb.text(
      `ðŸŽ¨ #${m.id} ${colorTag}`,
      `adm:pay:color:${m.id}`,
    ).text(
    ...
    // "Where TXID? / Where Order ID?" tutorial editor â€” same shape
    ...
    kb.text(`ðŸ“˜ #${m.id} Tutorial: ${tutTag}`, `adm:pay:tut:${m.id}`).row();
    kb.text(`ðŸ—‘ #${m.id} ${m.name}`.slice(0, 60), `adm:pay:del:${m.id}`).row();
  }
  backRow(kb);
  await ctx.editMessageText(lines.join('\n'), { parse_mode: 'Markdown', reply_markup: kb });
}
...
// Shown when the admin taps the `ðŸ“˜ #N Tutorial` row on the payment
...
async function showPaymentTutorialEditor(
  ...
  const lines = [
    `ðŸ“˜ *Tutorial â€” ${escapeMd(m.name)} (#${m.id})*`,
    '',
    `*Text:* ${t.text ? '`set`' : '_unset_'}`,
    `*File:* ${t.file_id ? '`' + (t.file_type ?? 'file') + '`' : '_unset_'}`,
    `*URL:* ${t.url ? '`' + t.url + '`' : '_unset_'}`,
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ“ Set Text', `adm:pay:tut:settxt:${methodId}`)
    .text('ðŸ§¹ Clear Text', `adm:pay:tut:clrtxt:${methodId}`)
    .row()
    .text('ðŸŽž Set File', `adm:pay:tut:setfile:${methodId}`)
    .text('ðŸ§¹ Clear File', `adm:pay:tut:clrfile:${methodId}`)
    .row()
    .text('ðŸ”— Set URL', `adm:pay:tut:seturl:${methodId}`)
    .text('ðŸ§¹ Clear URL', `adm:pay:tut:clrurl:${methodId}`)
    .row()
    .text('â¬…ï¸ Back to Methods', 'adm:pay');
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
...
adminBot.callbackQuery(/^adm:pay:tut:settxt:(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `ðŸ“ Send the *Tutorial* text for method #${id} now.`,
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:pay:tut:setfile:(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `ðŸŽž Send a photo, video, or document for method #${id}.`,
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:pay:tut:seturl:(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `ðŸ”— Send the tutorial *URL* (\`http://\` or \`https://\`) for method #${id}.`,
    { parse_mode: 'Markdown' },
  );
});
...
adminBot.callbackQuery(/^adm:pay:color:(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({ text: `Color â†’ ${next}` });
  await showPaymentList(ctx);
});
...
adminBot.callbackQuery(/^adm:pay:icon:(\d+)$/, async (ctx) => {
  ...
  await ctx.reply(
    [
      `ðŸŒŸ *Set icon for payment method #${id}*`,
      '',
      'Send the next emoji message â€” premium custom emojis are supported.',
      '',
...
async function showDepositList(ctx: AppCtx): Promise<void> {
  ...
  const lines = ['ðŸ’° *Pending Deposits*', ''];
  const kb = new InlineKeyboard();
  for (const d of deps) {
    const amountStr =
    ...
    kb.text(`ðŸ’² Set Amount #${d.id}`, `adm:dep:amt:${d.id}`).row();
    kb.text(`âœ… Approve #${d.id}`, `adm:dep:ok:${d.id}`)
      .text(`âŒ Reject #${d.id}`, `adm:dep:no:${d.id}`)
      .row();
    if (d.tx_hash) {
      kb.text(`ðŸ” Re-verify #${d.id}`, `adm:dep:rv:${d.id}`).row();
    }
  }
  ...
  // queue, NOT just the 20-row dashboard window â€” uses
  // listAllPendingDeposits() under the hood.
  kb.text('ðŸ§¹ Reject ALL Pending', 'adm:dep:nuke:confirm').row();
  backRow(kb);
  await ctx.editMessageText(lines.join('\n'), { parse_mode: 'Markdown', reply_markup: kb });
}
...
adminBot.callbackQuery('adm:dep:nuke:confirm', async (ctx) => {
  ...
  const kb = new InlineKeyboard()
    .text(`ðŸ§¨ Yes â€” reject all ${deps.length}`, 'adm:dep:nuke:do')
    .row()
    .text('â¬…ï¸ Cancel', 'adm:dep');
  await ctx.editMessageText(
    [
      'âš ï¸ *Reject ALL pending deposits?*',
      '',
      `This will mark every one of the *${deps.length}* pending deposit row(s) as *rejected* and DM each user.`,
      '',
      'This is a one-tap launch-cleanup tool â€” there is no per-row undo. Approved / already-rejected rows are left alone.',
    ].join('\n'),
    {
      parse_mode: 'Markdown',
      reply_markup: kb,
...
adminBot.callbackQuery('adm:dep:nuke:do', async (ctx) => {
  await ctx.answerCallbackQuery({ text: 'Rejectingâ€¦' });
  const deps = await listAllPendingDeposits();
  let rejected = 0;
  let errors = 0;
  for (const d of deps) {
  ...
  const summary = [
    'ðŸ§¹ *Bulk reject complete*',
    '',
    `â€¢ Rejected: *${rejected}*`,
    errors > 0 ? `â€¢ Errors: *${errors}* (see server logs)` : '',
    '',
...
adminBot.callbackQuery(/^adm:dep:amt:(\d+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    {
      parse_mode: 'Markdown',
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:dep'),
    },
  );
});
...
adminBot.callbackQuery(/^adm:dep:ok:(\d+)$/, async (ctx) => {
  ...
  if (Number(dep.amount) <= 0.01) {
    await ctx.answerCallbackQuery({
      text: 'Set the verified amount first via ðŸ’² Set Amount.',
      show_alert: true,
    });
    return;
  }
  ...
  try {
    await ctx.api.sendMessage(
      dep.user_id,
      `âœ… Your deposit *#${id}* of *$${dep.amount}* has been credited.\nNew balance: *$${newBal}*`,
      { parse_mode: 'Markdown' },
    );
  } catch (err) {
...
adminBot.callbackQuery(/^adm:dep:no:(\d+)$/, async (ctx) => {
  ...
  try {
    await ctx.api.sendMessage(
      dep.user_id,
      `âŒ Your deposit *#${id}* of *$${dep.amount}* was rejected. Please contact support.`,
      { parse_mode: 'Markdown' },
    );
  } catch (err) {
    logger.warn({ err }, 'Could not DM depositor');
...
adminBot.callbackQuery(/^adm:dep:rv:(\d+)$/, async (ctx) => {
  ...
  if (!dep.tx_hash) {
    await ctx.answerCallbackQuery({
      text: 'No tx hash stored â€” nothing to re-verify.',
      show_alert: true,
    });
    return;
  }
  await ctx.answerCallbackQuery({ text: 'Re-running auto-verifyâ€¦' });
  let result;
  try {
  ...
  if (result.ok) {
    try {
      await ctx.api.sendMessage(
        dep.user_id,
        `âœ… Your deposit *#${id}* of *$${result.amount.toFixed(2)}* has been credited.\nNew balance: *$${Number(result.newBalance).toFixed(2)}*`,
        { parse_mode: 'Markdown' },
      );
    } catch (err) {
      logger.warn({ err }, 'Could not DM depositor');
...
adminBot.callbackQuery('adm:cust', async (ctx) => {
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ“ Edit Text', 'adm:cust:text')
    .text('ðŸŽ¨ Set Color', 'adm:cust:color:pick')
    .row()
    .text('ðŸŽ¯ Custom Color Glyphs', 'adm:cust:colorglyph')
    .row()
    .text('ðŸ˜€ Set Emoji', 'adm:cust:emoji')
    .text('ðŸŽ¯ Set Button Icon', 'adm:cust:btnicon')
    .row()
    .text('ðŸ”— Set Channel URL', 'adm:cust:channel')
    .text('ðŸ” Reload Settings', 'adm:reload');
  backRow(kb);
...
adminBot.callbackQuery('adm:cust:channel', async (ctx) => {
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ—‘ Remove channel', 'adm:cust:channel:clear')
    .row()
    .text('â¬…ï¸ Back', 'adm:cust');
  await ctx.editMessageText(
...
function emojiStateLabel(key: string): string {
  ...
  // admin a clear picture: "ðŸ¯ + premium" / "ðŸ¯ (default)" / "not set".
  const spec = getEmoji(key);
  if (typeof spec === 'object' && spec.custom_emoji_id) {
    return `${spec.unicode} ${key} â€” premium`;
  }
  if (typeof spec === 'string' && spec !== key) {
    return `${spec} ${key}`;
  }
  return `Â· ${key} â€” not set`;
}
...
function emojiPickerKb(page: number): InlineKeyboard {
  ...
  if (totalPages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:cust:emoji:${page - 1}`);
    kb.text(`${page + 1}/${totalPages}`, 'adm:noop');
    if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:cust:emoji:${page + 1}`);
    kb.row();
  }
  kb.text('â¬…ï¸ Back', 'adm:cust');
  return kb;
}
...
async function showEmojiPicker(ctx: AppCtx, page: number): Promise<void> {
  const text =
    ...
    '(e.g. `ðŸ¯`) â€” or send a *premium emoji message* directly and the bot will ' +
...
adminBot.callbackQuery(/^adm:emoji:pick:(.+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    `ðŸ˜€ *Set Emoji* â€” \`${key}\`\n\n` +
      `${curLine}\n\n` +
      'Send any of:\n' +
      'â€¢ A plain unicode emoji â€” e.g. `ðŸ¯`\n' +
      'â€¢ A *premium* emoji message â€” the bot reads its `custom_emoji_id`\n' +
      'â€¢ Or the raw form: `<unicode> [custom_emoji_id]`\n\n' +
      'Or `/cancel`.',
    {
      parse_mode: 'Markdown',
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:cust:emoji'),
    },
  );
});
...
function buttonColorLabel(key: keyof typeof BUTTON_KEYS): string {
  // COLOR_PREFIX values are now empty (the old ðŸŸ¦ðŸŸ©ðŸŸ¥ðŸŸ¨ squares were
...
function colorPickerKb(page: number): InlineKeyboard {
  ...
  if (page === 0) {
    kb.text('ðŸ“‚ Product Categories â–¶ï¸', 'adm:catcolor:list').row();
  }
  for (const k of slice) {
    kb.text(
  ...
  if (totalPages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:cust:color:${page - 1}`);
    kb.text(`${page + 1}/${totalPages}`, 'adm:noop');
    if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:cust:color:${page + 1}`);
    kb.row();
  }
  kb.text('â¬…ï¸ Back', 'adm:cust');
  return kb;
...
adminBot.callbackQuery(/^adm:color:pick:(.+)$/, async (ctx) => {
  ...
  kb.row().text('â¬…ï¸ Back', 'adm:cust:color:pick');
  await ctx.editMessageText(`ðŸŽ¨ *Set Color* â€” \`${key}\`\n\nPick a color:`, {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
});
...
adminBot.callbackQuery(/^adm:color:set:([^:]+):([^:]+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({ text: `Set ${key} â†’ ${color}` });
  await showColorPicker(ctx, 0);
});
...
// explicitly themed â€” including any future categories the admin
...
async function showCategoryColorList(ctx: AppCtx, page: number): Promise<void> {
  ...
  if (page === 0) {
    ...
    kb.text(`âœ¨ Default (new categories) â€” ${def}`, 'adm:catcolor:pick:default').row();
  }
  for (const c of slice) {
    const color = getCategoryColor(c.id);
    const emoji = c.emoji ?? 'ðŸ“';
    const label = `${emoji} ${c.name} â€” ${color}`.slice(0, 60);
    kb.text(label, `adm:catcolor:pick:${c.id}`).row();
  }
  if (totalPages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:catcolor:list:${page - 1}`);
    kb.text(`${page + 1}/${totalPages}`, 'adm:noop');
    if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:catcolor:list:${page + 1}`);
    kb.row();
  }
  kb.text('â¬…ï¸ Back', 'adm:cust:color:pick');
  await ctx.editMessageText(
    'ðŸŽ¨ *Set Color â€” Product Categories*\n\n' +
...
adminBot.callbackQuery(/^adm:catcolor:pick:(default|\d+)$/, async (ctx) => {
  ...
  if (target === 'default') {
    ...
    label = cat ? `${cat.emoji ?? 'ðŸ“'} ${cat.name}` : `Category #${target}`;
  }
  const kb = new InlineKeyboard();
  for (const c of Object.keys(COLOR_PREFIX) as ColorMode[]) {
    kb.text(c, `adm:catcolor:set:${target}:${c}`);
  }
  kb.row().text('â¬…ï¸ Back', 'adm:catcolor:list');
  await ctx.editMessageText(`ðŸŽ¨ *Set Color* â€” ${label}\n\nPick a color:`, {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
});
...
adminBot.callbackQuery(/^adm:catcolor:set:(default|\d+):([^:]+)$/, async (ctx) => {
  ...
  if (target === 'default') {
    ...
    await ctx.answerCallbackQuery({ text: `Default â†’ ${color}` });
  } else {
    const id = Number(target);
    await setCategoryColor(id, color, ctx.from!.id);
    await ctx.answerCallbackQuery({ text: `#${id} â†’ ${color}` });
  }
  await showCategoryColorList(ctx, 0);
});
...
// matching coloured circle (ðŸ”µðŸŸ¢ðŸ”´ðŸŸ¡), but an admin can replace
...
async function showColorGlyphPicker(ctx: AppCtx): Promise<void> {
  ...
  const lines = ['ðŸŽ¯ *Custom Color Glyphs*', ''];
  ...
  const kb = new InlineKeyboard();
  for (const m of modes) {
    kb.text(`${m} â†’ ${getColorPrefix(m) || 'Â·'}`, `adm:colorglyph:edit:${m}`).row();
  }
  kb.text('â¬…ï¸ Back', 'adm:cust');
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
...
adminBot.callbackQuery(/^adm:colorglyph:edit:([^:]+)$/, async (ctx) => {
  ...
  await ctx.reply(
    `ðŸŽ¯ *Set Glyph for \`${mode}\`*\n\nCurrent: ${cur || '_(none)_'}\n\nSend a single emoji / symbol as your next message â€” it will become the new prefix glyph for the *${mode}* color mode. Send \`/clear\` to drop the override (falls back to the built-in default). Send \`/cancel\` to abort.`,
    { parse_mode: 'Markdown' },
  );
});

// ----- Button-icon picker (button-driven, A â†’ Z) -----
//
...
function buttonIconPickerKb(page: number): InlineKeyboard {
  ...
  if (totalPages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:cust:btnicon:${page - 1}`);
    kb.text(`${page + 1}/${totalPages}`, 'adm:noop');
    if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:cust:btnicon:${page + 1}`);
    kb.row();
  }
  kb.text('â¬…ï¸ Back', 'adm:cust');
  return kb;
...
async function showButtonIconPicker(ctx: AppCtx, page: number): Promise<void> {
  await ctx.editMessageText(
    'ðŸŽ¯ *Set Button Icon*\n\n' +
      ...
      '`custom_emoji_id` automatically. Each override is per-button â€” ' +
...
adminBot.callbackQuery(/^adm:btnicon:pick:(.+)$/, async (ctx) => {
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ—‘ Clear icon', `adm:btnicon:clear:${key}`)
    .row()
    .text('â¬…ï¸ Back', 'adm:cust:btnicon');
  await ctx.editMessageText(
    `ðŸŽ¯ *Set Button Icon* â€” \`${key}\`\n\n` +
      `${curLine}\n\n` +
      'Send a *premium emoji message* â€” the bot reads its ' +
...
//
// Flow:
//   1. `adm:ann`               â†’ reset to step:'text', prompt for body.
//   2. Admin sends text        â†’ step:'confirm' + render preview + the
//                                attach-Buy-button picker.
//   3. (optional) Buy button   â†’ product picker â†’ label / color / icon
//                                editor â†’ returns to confirm.
//   4. `adm:ann:send`          â†’ broadcast. If a Buy button is set, the
...
function announceConfirmKeyboard(recipients: number, buy?: AnnounceBuy): InlineKeyboard {
  const kb = new InlineKeyboard()
    .text(`ðŸ“£ Send to ${recipients}`, 'adm:ann:send')
    .row();
  if (buy) {
    kb.text('ðŸ›’ Edit Buy Button', 'adm:ann:buy:edit')
      .text('ðŸ—‘ Remove Button', 'adm:ann:buy:remove')
      .row();
  } else {
    kb.text('ðŸ›’ Add Buy Button', 'adm:ann:buy:add').row();
  }
  kb.text('âŒ Cancel', 'adm:root');
  return kb;
}
...
async function showAnnounceConfirm(ctx: AppCtx): Promise<void> {
  ...
  // Always normalize to step:'confirm' on entry â€” callers may have
  ...
  const buyLine = buy
    ? `\n\nðŸ›’ <b>Buy button:</b> <code>${escapeHtml(buy.label)}</code>` +
      `\n   â€¢ Product: <code>${escapeHtml(buy.product_name)}</code> (id=${buy.product_id})` +
      `\n   â€¢ Color: <code>${buy.color}</code>` +
      `\n   â€¢ Icon: ${buy.icon_unicode ? `${buy.icon_unicode} (premium)` : '<i>none</i>'}`
    : '\n\n<i>No Buy button attached. Tap â€œAdd Buy Buttonâ€ to deep-link an announcement to a specific product.</i>';
  try {
  ...
  await ctx.reply(`ðŸ“£ <b>Confirm broadcast</b>${buyLine}`, {
    parse_mode: 'HTML',
...
async function showAnnounceBuyProductPicker(ctx: AppCtx, page: number): Promise<void> {
  ...
  if (totalPages > 1) {
    if (page > 0) kb.text('â—€ï¸ Prev', `adm:ann:buy:prod:${page - 1}`);
    kb.text(`${page + 1}/${totalPages}`, 'adm:noop');
    if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:ann:buy:prod:${page + 1}`);
    kb.row();
  }
  kb.text('â¬…ï¸ Back', 'adm:ann:buy:cancel');
  await ctx.editMessageText(
    'ðŸ›’ *Pick the product the Buy button should open*\n\n' +
      'The button will deep-link straight to the product\'s quantity ' +
      'page in the bot â€” exactly what the user sees after tapping a ' +
      'product in the Shop.',
    { parse_mode: 'Markdown', reply_markup: kb },
  );
}
...
function announceBuyEditKeyboard(buy: AnnounceBuy): InlineKeyboard {
  const kb = new InlineKeyboard()
    .text('ðŸ“ Edit Label', 'adm:ann:buy:label')
    .row()
    .text(`ðŸŽ¨ Color: ${COLOR_PREFIX[buy.color] || 'âˆ…'} ${buy.color}`, 'adm:ann:buy:color')
    .row()
    .text(
      buy.icon_custom_emoji_id ? `âœ¨ Icon: ${buy.icon_unicode ?? ''} (set)` : 'âœ¨ Icon: (none)',
      'adm:ann:buy:icon',
    )
    .row()
    .text('ðŸ”„ Change Product', 'adm:ann:buy:add')
    .row()
    .text('ðŸ—‘ Remove Buy Button', 'adm:ann:buy:remove')
    .row()
    .text('âœ… Done', 'adm:ann:buy:done');
  return kb;
}
...
async function showAnnounceBuyEdit(ctx: AppCtx): Promise<void> {
  ...
  const previewText =
    `ðŸ›’ *Buy Button â€” preview*\n\n` +
    `â€¢ Product: \`${buy.product_name}\` (id=${buy.product_id})\n` +
    `â€¢ Label: \`${buy.label}\`\n` +
    `â€¢ Color: \`${buy.color}\`\n` +
    `â€¢ Icon: ${
...
function announceColorPickerKeyboard(): InlineKeyboard {
  ...
  for (const c of Object.keys(COLOR_PREFIX)) {
    kb.text(`${COLOR_PREFIX[c as ColorMode] || 'âˆ…'} ${c}`, `adm:ann:buy:color:${c}`);
  }
  kb.row().text('â¬…ï¸ Back', 'adm:ann:buy:edit');
  return kb;
}
...
adminBot.callbackQuery('adm:ann:buy:label', async (ctx) => {
  ...
  await ctx.editMessageText(
    `ðŸ“ *Edit Buy button label*\n\nCurrent: \`${buy.label}\`\n\n` +
      'Send the new label as a text message (max 64 chars). ' +
      'Premium custom emojis are supported â€” they\'ll render inline ' +
      'on premium clients and as their unicode fallback elsewhere.\n\n' +
      'Type `/cancel` to keep the current label.',
    {
      parse_mode: 'Markdown',
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:ann:buy:edit'),
    },
  );
});
...
adminBot.callbackQuery('adm:ann:buy:color', async (ctx) => {
  ...
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(
    `ðŸŽ¨ *Pick a button color* (Bot API 9.4 styles)\n\n` +
      `Current: ${COLOR_PREFIX[buy.color] || 'âˆ…'} \`${buy.color}\``,
...
adminBot.callbackQuery(/^adm:ann:buy:color:(.+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({ text: `Color â†’ ${color}` });
  ctx.session.adminFlow = {
    type: 'announce',
    step: 'confirm',
...
adminBot.callbackQuery('adm:ann:buy:icon', async (ctx) => {
  ...
  await ctx.editMessageText(
    'âœ¨ *Set Buy button icon*\n\n' +
      'Send a *premium* emoji message â€” the bot will read its ' +
...
adminBot.callbackQuery('adm:ann:send', async (ctx) => {
  ...
  ctx.session.adminFlow = undefined;
  await ctx.editMessageText(
    `ðŸ“£ Broadcast started for ${recipients.length} user(s).\n\nBot commands stay active while this runs.`,
    { reply_markup: backRow(new InlineKeyboard()) },
  );
  ...
  void (async () => {
    ...
    for (const r of recipients) {
      try {
        // Build a fresh keyboard per recipient â€” the underlying
    ...
    if (statusChatId && statusMessageId) {
      await api.editMessageText(
        statusChatId,
        statusMessageId,
        `âœ… Done. Delivered: *${ok}*, failed: *${fail}*.`,
        { parse_mode: 'Markdown', reply_markup: backRow(new InlineKeyboard()) },
...
async function showUserList(ctx: AppCtx, page: number): Promise<void> {
  ...
  const totalPages = Math.max(1, Math.ceil(total / PER_PAGE));
  // Rendered in HTML rather than Markdown V1 â€” the row text splices
  ...
  // `editMessageText` threw, and the ðŸ‘¥ Users button stayed stuck on
  // the loading spinner. HTML side-steps that â€” every user-supplied
  // string just goes through `escapeHtml`.
  const lines = [
    `ðŸ‘¥ <b>Users</b> â€” page ${page + 1}/${totalPages}  (total ${total})`,
    '',
  ];
  const kb = new InlineKeyboard();
  for (const u of rows) {
    ...
    const safeHandle = escapeHtml(handle);
    lines.push(
      `<code>${u.telegram_id}</code> ${safeHandle}  â€¢  $${Number(u.balance).toFixed(2)}`,
    );
  ...
  if (page > 0) kb.text('â—€ï¸ Prev', `adm:usr:${page - 1}`);
  if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:usr:${page + 1}`);
  kb.row().text('ðŸ” Find user', 'adm:usr:find').row().text('â¬…ï¸ Back', 'adm:root');
...
async function showUserCard(ctx: AppCtx, user: DBUser): Promise<void> {
  ...
  // does NOT honour `\_` escapes â€” so a username like `@lais_one` or
  ...
  // screens never opened. HTML has no such gotcha â€” we just need to
  ...
  const lines = [
    ...
    `Orders: <b>${summary.orders}</b> â€¢ Total spent: <b>$${summary.spent.toFixed(2)}</b>`,
    `Admin: ${isAdminUser ? 'âœ…' : 'âŒ'}`,
    user.is_banned
      ? `Banned: <b>YES</b>${
          user.banned_at
  ...
  const kb = new InlineKeyboard()
    .text('ðŸ’° Adjust balance', `adm:usr:bal:${user.telegram_id}`)
    .row();
  if (isAdminUser) {
    kb.text('ðŸ›¡ Demote admin', `adm:usr:demote:${user.telegram_id}`);
  } else {
    kb.text('ðŸ›¡ Promote admin', `adm:usr:promote:${user.telegram_id}`);
  }
  kb.row();
  // Admins can never be banned via this UI â€” promote-then-ban
  // would be self-defeating, so just hide the row entirely.
  if (!isAdminUser) {
    if (user.is_banned) {
      kb.text('â™»ï¸ Unban user', `adm:usr:unban:${user.telegram_id}`);
    } else {
      kb.text('ðŸš« Ban user', `adm:usr:ban:${user.telegram_id}`);
    }
    kb.row();
  }
  kb.text('ðŸ§¾ View Orders', `adm:ord:u:${user.telegram_id}:0`).row();
  kb.text('ðŸ’Ž Custom prices', `adm:price:u:${user.telegram_id}`).row();
  kb.text('â¬…ï¸ Back to users', 'adm:usr:0').text('ðŸ  Main', 'adm:root');
  if (ctx.callbackQuery) {
    await ctx.editMessageText(lines.join('\n'), { parse_mode: 'HTML', reply_markup: kb });
  } else {
...
adminBot.callbackQuery(/^adm:usr:promote:(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({ text: 'ðŸ›¡ Promoted to admin.' });
...
// One-tap unban â€” no extra prompt, mirrors how Promote/Demote work.
adminBot.callbackQuery(/^adm:usr:unban:(\d+)$/, async (ctx) => {
  const id = Number(ctx.match[1]);
  await unbanUser(id);
  await ctx.answerCallbackQuery({ text: 'â™»ï¸ User unbanned.' });
  const u = await findUserById(id);
  if (u) await showUserCard(ctx, u);
});

// ============================================================
// Referral Admin â€” view/correct available referral balances.
// ============================================================
...
async function showReferralAdminList(ctx: AppCtx, page: number): Promise<void> {
  ...
  for (const row of rows) {
    ...
    lines.push(
      `<code>${row.user.telegram_id}</code> ${escapeHtml(label)}  â€¢  ` +
        `available <b>${row.balance.available}</b> / total ${row.balance.total} / used ${row.balance.spent}`,
    );
    kb.text(
      `${label.slice(0, 18)} â€¢ ${row.balance.available} refs`,
      `adm:refs:v:${row.user.telegram_id}`,
    ).row();
  }
  if (rows.length === 0) lines.push('<i>No users found.</i>');
  if (page > 0) kb.text('â—€ï¸ Prev', `adm:refs:${page - 1}`);
  if (page + 1 < totalPages) kb.text('Next â–¶ï¸', `adm:refs:${page + 1}`);
  if (totalPages > 1) kb.row();
  kb.text('ðŸ” Find User', 'adm:refs:find')
    .text('â™»ï¸ Delete All Used Refs', 'adm:refs:resetall:ask')
    .row();
  kb.text('ðŸ”„ Refresh', `adm:refs:${page}`).text('â¬…ï¸ Back', 'adm:root');
...
async function showReferralAdminUser(ctx: AppCtx, user: DBUser): Promise<void> {
  ...
  const kb = new InlineKeyboard()
    .text('âž• +1 Ref', `adm:refs:adj:${user.telegram_id}:1`)
    .text('âž– -1 Ref', `adm:refs:adj:${user.telegram_id}:-1`)
    .row()
    .text('âœï¸ Custom +/-', `adm:refs:custom:${user.telegram_id}`)
    .row()
    .text('â™»ï¸ Delete User Used Refs', `adm:refs:reset:${user.telegram_id}:ask`)
    .row()
    .text('â¬…ï¸ Back to Referrals', 'adm:refs:0')
    .text('ðŸ  Main', 'adm:root');
  if (ctx.callbackQuery) {
    await ctx.editMessageText(lines.join('\n'), { parse_mode: 'HTML', reply_markup: kb });
  } else {
    await ctx.reply(lines.join('\n'), { parse_mode: 'HTML', reply_markup: kb });
...
// ============================================================
// Custom Prices â€” per-user, per-product price overrides.
//
// Flow:
//   adm:price            â†’ ask admin for telegram_id (or @username)
//   adm:price:u:<tgid>   â†’ render the user's override list with
//                          buttons to add a new override, edit /
//                          clear an existing one, bulk paste, or
//                          clear-all.
//   adm:price:add:<tgid> â†’ list every active product so the admin
//                          can pick which one to override next.
//   adm:price:set:<tgid>:<pid>
//                        â†’ prompt for the override price.
//   adm:price:del:<tgid>:<pid>
//                        â†’ drop a single override.
//   adm:price:bulk:<tgid> â†’ enter bulk-paste mode.
//   adm:price:clr:<tgid> â†’ drop every override for the user.
// ============================================================
...
async function showCustomPriceUserPick(ctx: AppCtx): Promise<void> {
  ...
  const body =
    ...
    'the bot yet â€” paste their numeric Telegram ID.\n\n' +
    'Or `/cancel`.';
  // Two top-level shortcuts that don't require typing a user first:
  //   ðŸ“Š Overview â€” paginated, deeply detailed table of every
  //                 override across every user.
  //   ðŸ“¥ Export CSV â€” same data as a downloadable .csv file you can
  //                   open in Excel / Google Sheets for sorting,
  //                   filtering, charting.
  const kb = new InlineKeyboard()
    .text('ðŸ“Š Full overview', 'adm:price:report:0')
    .text('ðŸ“¥ Export CSV', 'adm:price:csv')
    .row()
    .text('â¬…ï¸ Back', 'adm:root');
  if (ctx.callbackQuery) {
...
async function showCustomPriceUserCard(
  ...
  // Rendered in HTML rather than Markdown V1 â€” V1 treats `_` as
  ...
  const lines: string[] = [
    `ðŸ’Ž <b>Custom Prices for</b> <code>${telegram_id}</code> ` +
  ...
  const kb = new InlineKeyboard()
    .text('âž• Add / edit override', `adm:price:add:${telegram_id}:0`)
    .row()
    .text('ðŸ“‹ Bulk paste', `adm:price:bulk:${telegram_id}`)
    .row();
  if (overrides.length > 0) {
    // Clear-rows: each override gets a one-tap delete row.
    for (const o of overrides) {
      kb.text(
        `ðŸ—‘ ${o.product_name.slice(0, 40)} ($${o.price.toFixed(2)})`,
        `adm:price:del:${telegram_id}:${o.product_id}`,
      ).row();
    }
    kb.text('ðŸ§¹ Clear ALL overrides', `adm:price:clr:${telegram_id}`).row();
  }
  kb.text('â¬…ï¸ Back', 'adm:price');
  if (ctx.callbackQuery) {
    await ctx.editMessageText(lines.join('\n'), {
      parse_mode: 'HTML',
      reply_markup: kb,
...
async function showCustomPriceProductPicker(
  ...
  const lines = [
    `ðŸ’Ž *Pick a product* â€” page ${safePage + 1}/${totalPages}`,
    '',
  ...
  for (const p of rows) {
    ...
    const label =
      cur !== undefined
        ? `${p.name} â€” $${cur.toFixed(2)} (was $${Number(p.price).toFixed(2)})`
        : `${p.name} â€” $${Number(p.price).toFixed(2)}`;
    kb.text(label.slice(0, 60), `adm:price:set:${telegram_id}:${p.id}`).row();
  }
  if (safePage > 0) {
    kb.text('â—€ Prev', `adm:price:add:${telegram_id}:${safePage - 1}`);
  }
  if (safePage + 1 < totalPages) {
    kb.text('Next â–¶', `adm:price:add:${telegram_id}:${safePage + 1}`);
  }
  kb.row().text('â¬…ï¸ Back', `adm:price:u:${telegram_id}`);
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
...
adminBot.callbackQuery(/^adm:price:del:(\d+):(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({ text: 'ðŸ—‘ Override removed.' });
...
adminBot.callbackQuery(/^adm:price:clr:(\d+)$/, async (ctx) => {
  ...
  await ctx.answerCallbackQuery({
    text: n === 0 ? 'No overrides to clear.' : `ðŸ§¹ Cleared ${n} overrides.`,
  });
  await showCustomPriceUserCard(ctx, telegram_id);
});
...
adminBot.callbackQuery(/^adm:price:bulk:(\d+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    `ðŸ“‹ *Bulk paste* â€” for user \`${telegram_id}\`\n\n` +
...
// ------------------------------------------------------------
// ðŸ“Š Full overview â€” paginated, deeply detailed table of every
...
// product with default â†’ override price + delta.
//
...
adminBot.callbackQuery(/^adm:price:report:(\d+)$/, async (ctx) => {
  ...
  if (all.length === 0) {
    await ctx.editMessageText(
      'ðŸ“Š *Custom Prices â€” Overview*\n\n_No overrides set yet._\n\n' +
        'Add one via the user pick screen or pre-set by Telegram ID.',
      {
        parse_mode: 'Markdown',
        reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:price'),
      },
    );
    return;
  }
  ...
  const header = [
    'ðŸ“Š *Custom Prices â€” Overview*',
    `Users with overrides: *${groupArr.length}* Â· ` +
      `Total override rows: *${all.length}*` +
      ` Â· Page ${safePage + 1}/${totalPages}`,
    '',
  ];
    ...
    const swingTxt =
      swing === 0
        ...
        : swing > 0
          ...
          : `âˆ’$${Math.abs(swing).toFixed(2)} below default`;
    const userLine =
      `*${i + 1}.* \`${g.telegram_id}\` _(${escapeHtml(handle)})_ Â· ` +
      `*${g.rows.length}* override${g.rows.length === 1 ? '' : 's'} Â· ${swingTxt}`;
    const productLines = g.rows.map((r) => {
      ...
      const pct =
        r.product_default_price > 0
          ...
          : 'n/a';
      return (
        `   â€¢ \`#${r.product_id}\` ${escapeHtml(r.product_name)}: ` +
        `*$${r.price.toFixed(2)}* ` +
  ...
  const kb = new InlineKeyboard();
  if (safePage > 0) {
    kb.text('â—€ Prev', `adm:price:report:${safePage - 1}`);
  }
  if (safePage + 1 < totalPages) {
    kb.text('Next â–¶', `adm:price:report:${safePage + 1}`);
  }
  kb.row().text('ðŸ“¥ Export CSV', 'adm:price:csv').row();
  kb.text('â¬…ï¸ Back', 'adm:price');
...
// ðŸ“¥ CSV export â€” emits the same data as a downloadable file with
...
adminBot.callbackQuery('adm:price:csv', async (ctx) => {
  await ctx.answerCallbackQuery({ text: 'Building CSVâ€¦' });
  const all = await listAllPriceOverrides();
  if (all.length === 0) {
    await ctx.reply(
      'ðŸ“¥ No overrides to export â€” set at least one before downloading.',
    );
    return;
  }
  const header = [
...
// ============================================================
// ðŸ’¸ Promos â€” qty-threshold flat-USDT auto-discounts.
//
// Hierarchical scope: per-user-per-product â†’ per-user â†’ per-product
// â†’ default. Most specific match wins; ties on the same tier go to
// the largest discount. Resolution + integration live in
// `src/services/promo.ts` and the `pay:wallet:<id>` handler â€” this
// section is purely the admin CRUD UI.
//
// Routes:
//   adm:promo                     â†’ list page 0
//   adm:promo:list:<page>         â†’ paginated list
//   adm:promo:v:<id>              â†’ view/edit a single promo
//   adm:promo:new                 â†’ start the new-promo wizard
//   adm:promo:scope:<scope>       â†’ scope chosen
//   adm:promo:np:<page>           â†’ product picker (new promo)
//   adm:promo:npp:<product_id>    â†’ product chosen
//   adm:promo:nameSkip            â†’ skip optional promo name
//   adm:promo:toggle:<id>         â†’ flip active flag
//   adm:promo:editq:<id>          â†’ start "change min qty" prompt
//   adm:promo:editd:<id>          â†’ start "change discount" prompt
//   adm:promo:editn:<id>          â†’ start "change name" prompt
//   adm:promo:del:<id>            â†’ delete confirmation
//   adm:promo:delok:<id>          â†’ confirm delete
// ============================================================
...
function promoScopeLabel(p: Pick<DBPromo, 'product_id' | 'telegram_id'> & { product_name?: string | null }): string {
  if (p.telegram_id !== null && p.product_id !== null) {
    return `User \`${p.telegram_id}\` Â· Product ${p.product_name ? `*${p.product_name}*` : `#${p.product_id}`}`;
  }
  if (p.telegram_id !== null) return `User \`${p.telegram_id}\` Â· _any product_`;
  if (p.product_id !== null) return `_any user_ Â· Product ${p.product_name ? `*${p.product_name}*` : `#${p.product_id}`}`;
  return '_default â€” every user, every product_';
}
...
async function showPromoList(ctx: AppCtx, page: number): Promise<void> {
  ...
  const lines: string[] = [
    'ðŸ’¸ *Promos*',
    '',
    `Active rule set â€” page ${safePage + 1}/${totalPages}  (total ${total})`,
  ];
  if (rows.length === 0) {
    lines.push('', '_No promos yet._', 'Tap *âž• Add promo* to create the first one.');
  } else {
    lines.push('');
    for (const p of rows) {
      const scope = promoScopeLabel(p);
      const active = p.active ? 'ðŸŸ¢' : 'â¸';
      const name = p.name?.trim() ? ` â€” _${p.name.trim()}_` : '';
      lines.push(
        `${active} \`#${p.id}\` qty â‰¥ *${p.min_qty}* â†’ âˆ’*$${Number(p.discount_amount).toFixed(2)}*${name}`,
        `   ${scope}`,
      );
    }
  }
  const kb = new InlineKeyboard()
    .text('âž• Add promo', 'adm:promo:new')
    .row()
    .text('ðŸ“Š Full overview', 'adm:promo:report:0')
    .row();
  for (const p of rows) {
    const scope = p.telegram_id && p.product_id
      ...
      : p.telegram_id
        ...
        : p.product_id
          ...
          : 'Def';
    kb.text(
      `#${p.id} ${scope} qâ‰¥${p.min_qty} âˆ’$${Number(p.discount_amount).toFixed(2)}`,
      `adm:promo:v:${p.id}`,
    ).row();
  }
  if (safePage > 0) kb.text('â—€ Prev', `adm:promo:list:${safePage - 1}`);
  if (safePage + 1 < totalPages) kb.text('Next â–¶', `adm:promo:list:${safePage + 1}`);
  kb.row().text('â¬…ï¸ Back', 'adm:root');
...
/**
 * `/promo [add|list|edit <id>|delete <id>]` â€” convenience slash
 * command that lands on the same UI as the panel button. Subcommands
 * are best-effort shortcuts:
 *   /promo            â†’ list page 0
 *   /promo list       â†’ list page 0
 *   /promo add        â†’ start the new-promo wizard (scope picker)
 *   /promo edit <id>  â†’ jump to the promo card (toggle / edit / delete)
 *   /promo delete <id>â†’ jump to the delete confirmation
 */
adminBot.command('promo', async (ctx) => {
  ...
  if (/^add\b/i.test(arg)) {
    const kb = new InlineKeyboard()
      .text('ðŸŒ Default (everyone, every product)', 'adm:promo:scope:default')
      .row()
      .text('ðŸ“¦ Per product (any user)', 'adm:promo:scope:product')
      .row()
      .text('ðŸ‘¤ Per user (any product)', 'adm:promo:scope:user')
      .row()
      .text('ðŸ‘¤ðŸ“¦ Per user + product', 'adm:promo:scope:user_product')
      .row()
      .text('â¬…ï¸ Cancel', 'adm:promo');
    await ctx.reply(
      [
        'âž• *New promo â€” Step 1/4: Scope*',
        '',
  ...
  if (delMatch) {
    ...
    if (!p) {
      await ctx.reply(`â“ Promo #${id} not found.`);
      return;
    }
    const kb = new InlineKeyboard()
      .text('ðŸ—‘ Yes, delete', `adm:promo:delok:${id}`)
      .text('âŒ Cancel', `adm:promo:v:${id}`);
    await ctx.reply(
  ...
  await ctx.reply(
    [
      'ðŸ’¸ *Usage:*',
      '`/promo`              â€” list promos',
      '`/promo add`          â€” new promo wizard',
      '`/promo edit <id>`    â€” open a promo',
      '`/promo delete <id>`  â€” delete a promo',
    ].join('\n'),
    { parse_mode: 'Markdown' },
  );
});
 ...
 * Custom Prices "user card" â€” beyond the raw row, also surfaces
...
async function showPromoCard(ctx: AppCtx, promo_id: number): Promise<void> {
  ...
  if (!p) {
    await sendOrEdit(
      ctx,
      'â“ Promo not found.',
      new InlineKeyboard().text('â¬…ï¸ Back', 'adm:promo'),
    );
    return;
  }
  // Hydrate side-tables in parallel â€” none of them block on each
  ...
  const lines: string[] = [
    `ðŸ’¸ *Promo #${p.id}*`,
    '',
    `Status: ${p.active ? 'ðŸŸ¢ *Active*' : 'â¸ *Paused*'}`,
    `Scope: ${scope}`,
    `Min qty: *${p.min_qty}*`,
    `Discount: *$${Number(p.discount_amount).toFixed(2)}* off the line total`,
    `Name: ${p.name?.trim() ? `_${p.name.trim()}_` : 'â€”'}`,
  ];
  ...
  if (product) {
    ...
    lines.push(
      '',
      `ðŸ“¦ *Product*`,
      `   â€¢ \`#${product.id}\` ${escapeHtml(product.name)}`,
      `   â€¢ Default price: *$${price.toFixed(2)}*`,
      `   â€¢ In stock: *${stock}*`,
    );
    // Cheap "savings preview" so the admin can sanity-check the
    // promo at a glance â€” what the buyer pays at the threshold qty.
    ...
    lines.push(
      `   â€¢ At qty *${p.min_qty}*: gross *$${grossAtMin.toFixed(2)}*` +
        ` â†’ pay *$${totalAtMin.toFixed(2)}* (saves $${discountAtMin.toFixed(2)} / ${pct})`,
    );
  }
  ...
  if (targetUser) {
    ...
    lines.push(
      '',
      `ðŸ‘¤ *Target user*`,
      `   â€¢ \`${targetUser.telegram_id}\` _(${escapeHtml(handle)})_`,
      `   â€¢ Wallet balance: *$${balance.toFixed(2)}*`,
    );
    ...
    lines.push(
      '',
      `ðŸ‘¤ *Target user*`,
      `   â€¢ \`${p.telegram_id}\` _(hasn't started the bot)_`,
    );
  }

  // Excluded users â€” opt-out list, applied on top of the scope
  ...
  if (excluded.length > 0) {
    ...
    lines.push(
      '',
      `ðŸš« *Excluded users (${excluded.length})*`,
      `   ${preview}${more}`,
    );
  } else {
    lines.push('', `ðŸš« *Excluded users:* _none_`);
  }

  // Impact stats â€” only meaningful once at least one order has used
  // this promo, but we always render the section so the admin sees
  // "0 orders so far" for new rules.
  lines.push(
    '',
    `ðŸ“Š *Impact (paid orders)*`,
    `   â€¢ Orders matched: *${impact.orders}*`,
    `   â€¢ Total discount given: *$${impact.total_discount.toFixed(2)}*`,
    `   â€¢ Last used: ${impact.last_used ? new Date(impact.last_used).toLocaleString('en-GB') : 'â€”'}`,
  );
  ...
  const kb = new InlineKeyboard()
    .text(p.active ? 'â¸ Pause' : 'â–¶ï¸ Activate', `adm:promo:toggle:${p.id}`)
    .row()
    .text('âœï¸ Min qty', `adm:promo:editq:${p.id}`)
    .text('ðŸ’µ Discount', `adm:promo:editd:${p.id}`)
    .row()
    .text('ðŸ· Name', `adm:promo:editn:${p.id}`)
    .text(
      excluded.length > 0
        ? `ðŸš« Excluded (${excluded.length})`
        : 'ðŸš« Exclude users',
      `adm:promo:ex:${p.id}`,
    )
    .row()
    .text('ðŸ—‘ Delete', `adm:promo:del:${p.id}`)
    .row()
    .text('â¬…ï¸ Back to list', 'adm:promo');
  await sendOrEdit(ctx, lines.join('\n'), kb);
}
...
// `findApplicablePromos` and `findScopedActivePromos` â€” this is
// purely the CRUD UI.
//
// Routes:
//   adm:promo:ex:<id>             â†’ list excluded users for promo
//   adm:promo:exadd:<id>          â†’ start "exclude user" prompt
//   adm:promo:exdel:<id>:<tg>     â†’ un-exclude a user
...
async function showPromoExclusions(ctx: AppCtx, promo_id: number): Promise<void> {
  ...
  const excluded = Array.isArray(p.excluded_telegram_ids)
    ? p.excluded_telegram_ids.map(Number)
    : [];
  // Hydrate handles for the listed users â€” same pattern as the
  ...
  const lines: string[] = [
    `ðŸš« *Promo #${p.id} â€” Excluded users*`,
    '',
    `Promo: qty â‰¥ *${p.min_qty}* â†’ âˆ’*$${Number(p.discount_amount).toFixed(2)}* ` +
      `(${p.active ? 'ðŸŸ¢ Active' : 'â¸ Paused'})`,
  ...
  if (handles.length === 0) {
    lines.push(
      ...
      '',
      'Tap *âž• Exclude a user* to opt someone out of this promo.',
      'They will keep seeing other promos that apply to them â€” only',
      'this specific rule will be skipped at checkout.',
    );
  } else {
    lines.push(`*${handles.length}* user${handles.length === 1 ? '' : 's'} opted out:`);
    for (const h of handles) {
      lines.push(`   â€¢ \`${h.id}\` â€” ${escapeHtml(h.label)}`);
    }
  }

  const kb = new InlineKeyboard()
    .text('âž• Exclude a user', `adm:promo:exadd:${p.id}`)
    .row();
  for (const h of handles) {
    kb.text(`ðŸ—‘ Remove ${h.id}`, `adm:promo:exdel:${p.id}:${h.id}`).row();
  }
  kb.text('â¬…ï¸ Back to promo', `adm:promo:v:${p.id}`);
  await sendOrEdit(ctx, lines.join('\n'), kb);
}
...
adminBot.callbackQuery(/^adm:promo:exadd:(\d+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    [
      `ðŸš« *Promo #${id} â€” Exclude a user*`,
      '',
      'Send the *numeric Telegram ID* (e.g. `123456789`) or `@username`',
      'of the user to opt out of this promo. They will still be able',
      'to buy the product â€” only this promo will be skipped for them.',
      '',
      '`/cancel` to abort.',
    ].join('\n'),
    {
      parse_mode: 'Markdown',
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Cancel', `adm:promo:ex:${id}`),
    },
  );
});
...
adminBot.callbackQuery(/^adm:promo:exdel:(\d+):(\d+)$/, async (ctx) => {
  ...
  try {
    ...
    await ctx.answerCallbackQuery({
      text: 'Could not un-exclude â€” try again.',
      show_alert: true,
    });
  }
...
//
// Mirrors `adm:price:report` â€” groups every promo into the four
...
const PROMO_REPORT_TIERS = [
  { key: 'user_product', label: 'ðŸ‘¤ðŸ“¦ Per user + product', tier: 3 },
  { key: 'user', label: 'ðŸ‘¤ Per user (any product)', tier: 2 },
  { key: 'product', label: 'ðŸ“¦ Per product (any user)', tier: 1 },
  { key: 'default', label: 'ðŸŒ Default (everyone, every product)', tier: 0 },
] as const;

const PROMO_REPORT_PER_PAGE = 4; // 4 tier blocks per page max â€” keeps under Telegram's 4096-char limit comfortably.
...
adminBot.callbackQuery(/^adm:promo:report:(\d+)$/, async (ctx) => {
  ...
  if (all.length === 0) {
    await ctx.editMessageText(
      'ðŸ“Š *Promos â€” Overview*\n\n_No promos yet._\n\nTap *âž• Add promo* on the previous screen.',
      {
        parse_mode: 'Markdown',
        reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:promo'),
      },
    );
    return;
  }
  ...
  // Group by tier â€” already sorted by created_at desc inside listAllPromos.
  ...
  const header = [
    'ðŸ“Š *Promos â€” Overview*',
    `Promos: *${all.length}* (active *${totalActive}*) Â· ` +
  ...
  for (let i = start; i < end; i++) {
    ...
    const lines: string[] = [
      `${tierMeta.label}`,
      `   _${rows.length} rule${rows.length === 1 ? '' : 's'} Â· ` +
        `${tierImpact.orders} orders Â· ` +
        `$${tierImpact.total_discount.toFixed(2)} given_`,
    ];
    for (const r of rows) {
      const status = r.active ? 'ðŸŸ¢' : 'â¸';
      const name = r.name?.trim() ? ` â€” _${r.name.trim()}_` : '';
      const userPart =
        r.telegram_id !== null
          ? ` user \`${r.telegram_id}\`${
      ...
      const productPart =
        r.product_id !== null
          ? ` Â· *${escapeHtml(r.product_name ?? `#${r.product_id}`)}*`
          : '';
      const im = impactMap.get(r.id)!;
      lines.push(
        `   â€¢ ${status} \`#${r.id}\` qty â‰¥ *${r.min_qty}* â†’ âˆ’*$${Number(r.discount_amount).toFixed(2)}*${name}` +
          `${userPart}${productPart}`,
        `       impact: ${im.orders} orders Â· $${im.total_discount.toFixed(2)}`,
      );
    }
  ...
  const kb = new InlineKeyboard();
  if (safePage > 0) kb.text('â—€ Prev', `adm:promo:report:${safePage - 1}`);
  if (safePage + 1 < totalPages) kb.text('Next â–¶', `adm:promo:report:${safePage + 1}`);
  kb.row().text('ðŸ“¥ Export CSV', 'adm:promo:csv').row().text('â¬…ï¸ Back', 'adm:promo');
...
adminBot.callbackQuery('adm:promo:csv', async (ctx) => {
  ...
  const all = await listAllPromos();
  if (all.length === 0) {
    await ctx.reply('ðŸ“¥ No promos to export â€” add at least one before downloading.');
    return;
  }
  const impactArr = await Promise.all(all.map((p) => getPromoImpact(p.id)));
  const header = [
...
async function showPromoNewProductPicker(
  ...
  if (flow?.type !== 'promo_add' || flow.step !== 'pick_product') {
    await ctx.editMessageText(
      'â“ Lost the promo flow â€” start over.',
      { reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:promo') },
    );
    return;
  }
  ...
  for (const p of rows) {
    kb.text(
      `${p.name.slice(0, 40)} â€” $${Number(p.price).toFixed(2)}`,
      `adm:promo:npp:${p.id}`,
    ).row();
  }
  if (safePage > 0) kb.text('â—€ Prev', `adm:promo:np:${safePage - 1}`);
  if (safePage + 1 < totalPages) kb.text('Next â–¶', `adm:promo:np:${safePage + 1}`);
  kb.row().text('â¬…ï¸ Cancel', 'adm:promo');
  await ctx.editMessageText(lines.join('\n'), {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
...
async function promptPromoMinQty(ctx: AppCtx): Promise<void> {
  await ctx.editMessageText(
    [
      'âž• *New promo â€” Step 3/4: Minimum qty*',
      '',
      'Send the minimum quantity that triggers this promo.',
      'Whole number â‰¥ 1, e.g. `10`.',
      '',
      'Send `/cancel` to abort.',
    ].join('\n'),
    {
      parse_mode: 'Markdown',
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Cancel', 'adm:promo'),
    },
  );
}
...
adminBot.callbackQuery(/^adm:promo:scope:(default|product|user|user_product)$/, async (ctx) => {
  ...
  // user / user_product â†’ first ask for telegram id or @username.
  ctx.session.adminFlow = {
    type: 'promo_add',
    step: 'pick_user',
    data: { scope },
  ...
  await ctx.editMessageText(
    [
      'âž• *New promo â€” Step 2: Pick user*',
      '',
...
adminBot.callbackQuery(/^adm:promo:npp:(\d+)$/, async (ctx) => {
  ...
  const flow = ctx.session.adminFlow;
  if (flow?.type !== 'promo_add' || flow.step !== 'pick_product') {
    await ctx.editMessageText('â“ Lost the promo flow â€” start over.', {
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:promo'),
    });
    return;
  }
...
adminBot.callbackQuery(/^adm:promo:editq:(\d+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    `âœï¸ *Promo #${id} â€” Min qty*\n\nSend the new minimum quantity (whole number â‰¥ 1).\n\n\`/cancel\` to abort.`,
    {
      parse_mode: 'Markdown',
      reply_markup: new InlineKeyboard().text('â¬…ï¸ Cancel', `adm:promo:v:${id}`),
    },
  );
});
...
adminBot.callbackQuery('adm:promo:nameSkip', async (ctx) => {
  ...
  try {
    ...
    const detail =
      e?.code === '42P01'
        ? 'The `promos` table is missing â€” apply migrations 0013 / 0014.'
        : (e?.message ?? String(err)).slice(0, 500);
    await ctx.editMessageText(
      `âš ï¸ *Could not save promo*\n\n\`${escapeHtml(detail)}\``,
      {
        parse_mode: 'Markdown',
        reply_markup: new InlineKeyboard().text('â¬…ï¸ Back', 'adm:promo'),
      },
  ...
  try {
    ...
    await ctx.reply(
      `âœ… Promo *#${created.id}* saved.\n\n_(Detail card render failed â€” open it from /promo list.)_`,
...
// ============================================================
// Multi-step input handler â€” fired for any text msg from admin
...
adminBot.on('message:text', async (ctx, next) => {
  ...
  // marker on the text we hand off to flow handlers â€” `renderMdHtml`
  ...
  if (text === '/cancel') {
    ctx.session.adminFlow = undefined;
    await ctx.reply('âŒ Cancelled.', { reply_markup: rootMenu() });
    return;
  }
  ...
  try {
    if (flow.type === 'supplier_reseller_add') {
      ...
      const key =
        ...
        rawKey;
      if (key.length < 24) {
        await ctx.reply('âŒ Send the full reseller API key, or `/cancel`.', {
          parse_mode: 'Markdown',
        });
        return;
      }
      ...
      try {
        ...
        testLine = test.ok
          ? `Live test OK: ${test.balance === null ? 'balance unknown' : `balance ${apiMoney(test.balance)}`} Â· ${test.productsSeen} products`
      ...
      await ctx.reply(
        `âœ… Reseller API supplier saved: *${escapeMd(source.name)}* (#${source.id})\n\n${escapeMd(testLine)}\n\nTap *Browse Products* to import by button.`,
        { parse_mode: 'Markdown' },
      );
    ...
    if (flow.type === 'supplier_canboso_add') {
      ...
      if (key.length < 12) {
        await ctx.reply('âŒ Send the full Canboso API key, or `/cancel`.', {
          parse_mode: 'Markdown',
        });
        return;
      }
      ...
      await ctx.reply(
        `âœ… Canboso supplier saved: *${escapeMd(source.name)}* (#${source.id})\n\n${escapeMd(testLine)}\n\nTap *Browse Products* to import by button.`,
        { parse_mode: 'Markdown' },
      );
      await showSupplierDetail(ctx, source.id);
      return;
    ...
    if (flow.type === 'supplier_api_add') {
      ...
      try {
        const test = await testSupplierConnection(source);
        testLine = test.ok
          ? `Test OK: ${test.balance === null ? 'balance â€”' : `balance ${apiMoney(test.balance)}`} Â· ${test.productsSeen} products`
      ...
      await ctx.reply(
        `âœ… Supplier API saved: *${escapeMd(source.name)}* (#${source.id})\n\n${escapeMd(testLine)}`,
        { parse_mode: 'Markdown' },
      );
      await showSupplierDetail(ctx, source.id);
      return;
    ...
    if (flow.type === 'supplier_product_link_add') {
      ...
      if (!source) {
        await ctx.reply('âŒ Supplier not found. Check `supplier_id` and try again.', {
          parse_mode: 'Markdown',
        });
        return;
      }
      ...
      if (!product) {
        await ctx.reply('âŒ Local product not found. Check `local_product_id` and try again.', {
          parse_mode: 'Markdown',
        });
        return;
      }
    ...
    if (flow.type === 'add_category') {
      if (flow.step === 'name') {
        ...
        cache.del('cats');
        await ctx.reply(
          `âœ… Category *${cat.name}* added (id=${cat.id}).`,
          { parse_mode: 'Markdown', reply_markup: rootMenu() },
        );
      }
      return;
    ...
    if (flow.type === 'add_product') {
      if (flow.step === 'name') {
        ...
        if (!Number.isFinite(price) || price < 0) {
          await ctx.reply('âŒ Bad price. Send a number like `9.99`.');
          return;
        }
        ctx.session.adminFlow = {
          type: 'add_product',
        ...
        const kb = new InlineKeyboard()
          .text('â™¾ Unlimited', 'adm:prod:unl:yes')
          .text('ðŸ”¢ Set Count', 'adm:prod:unl:no');
        await ctx.reply(
          [
            '*Unlimited stock?*',
            '',
        ...
        // Free-form text in this step is unexpected â€” the user is
        ...
        if (!Number.isInteger(stock) || stock < 0) {
          await ctx.reply('âŒ Bad stock. Send an integer â‰¥ 0.');
          return;
        }
        ctx.session.adminFlow = {
          type: 'add_product',
        ...
        await ctx.reply(
          'Send the *View Note* text shown when buyer taps ðŸ“ View Note (or Skip).',
          { parse_mode: 'Markdown', reply_markup: kb },
        );
        ...
        await ctx.reply(
          [
            'ðŸ“¦ *Send the deliverables (items pool)* â€” one payload per line.',
    ...
    if (flow.type === 'edit_product_emoji') {
      ...
      // a text message â€” the visible text is the unicode fallback,
      ...
      if (!ent) {
        await ctx.reply(
          'âŒ I didn\'t see a premium emoji in that message. Try again with a single premium emoji.',
        );
        return;
      }
      await updateProduct(flow.data.product_id, { emoji_id: ent.custom_emoji_id });
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Premium emoji saved.');
    ...
    if (flow.type === 'edit_product_note_text') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Note text saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_description') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Description saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_warranty') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Warranty saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_tutorial_text') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Tutorial text saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_tutorial_url') {
      if (!/^https?:\/\//.test(text)) {
        await ctx.reply('âŒ URL must start with `http://` or `https://`.');
        return;
      }
      await updateProduct(flow.data.product_id, { tutorial_url: text });
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Tutorial URL saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_items') {
      // Bulk-add staging flow â€” accumulate payloads across many
      ...
      if (payloads.length === 0) {
        await ctx.reply(
          'âŒ No payloads found in that message. Send one payload per line, or tap *Cancel* on the staging card to exit.',
          { parse_mode: 'Markdown' },
        );
        return;
      }
      ...
      const note =
        accepted.length < payloads.length
          ? `Capped at ${ITEMS_STAGING_CAP} â€” ${payloads.length - accepted.length} line(s) were dropped. Tap Confirm to flush, then add more.`
          : undefined;
    ...
    if (flow.type === 'edit_product_price') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Price updated.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_stock') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Stock updated.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    ...
    if (flow.type === 'edit_product_referral_required') {
      ...
      if (!Number.isInteger(count) || count < 0) {
        await ctx.reply('âŒ Bad number. Send an integer â‰¥ 0.');
        return;
      }
      await updateProduct(flow.data.product_id, { referral_required_count: count });
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        count > 0 ? `âœ… Referral Pay set: ${count} referrals required.` : 'âœ… Referral Pay disabled.',
      );
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_id') {
      ...
      if (!Number.isInteger(newId) || newId <= 0) {
        await ctx.reply('âŒ Bad ID. Send a positive integer.');
        return;
      }
      const existing = await getProduct(newId);
      if (existing) {
        await ctx.reply(`âŒ Product with ID ${newId} already exists (*${escapeMd(existing.name)}*). Choose a different ID.`, { parse_mode: 'Markdown' });
        return;
      }
      await changeProductId(flow.data.product_id, newId);
      ctx.session.adminFlow = undefined;
      await ctx.reply(`âœ… Product ID changed: ${flow.data.product_id} â†’ ${newId}`);
    ...
    if (flow.type === 'edit_product_delivery_instruction') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(cleared ? 'âœ… Instruction reset to default.' : 'âœ… Instruction saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_delivery_success') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(cleared ? 'âœ… Success card reset to default.' : 'âœ… Success card saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_delivery_fields') {
      if (text.trim().toLowerCase() === 'clear') {
        ...
        ctx.session.adminFlow = undefined;
        await ctx.reply('âœ… All fields cleared.');
        await showProductEditor(ctx, flow.data.product_id, flow.data.page);
        return;
      }
      ...
      for (const line of rawLines) {
        ...
        if (parts.length < 2 || !parts[0] || !parts[1]) {
          errors.push(`â€¢ \`${line}\` â€” expected \`key | Label | required\``);
          continue;
        }
        const key = parts[0].toLowerCase().replace(/[^a-z0-9_]+/g, '_');
        if (!key) {
          errors.push(`â€¢ \`${line}\` â€” key must contain alphanumerics.`);
          continue;
        }
        const required =
          parts[2] === undefined
      ...
      if (errors.length > 0) {
        await ctx.reply(
          ['âŒ Could not parse:', ...errors, '', 'Try again or send `/cancel`.'].join('\n'),
          { parse_mode: 'Markdown' },
        );
        return;
      }
      if (fields.length === 0) {
        await ctx.reply('âŒ No valid fields found. Send `clear` to wipe the spec or `/cancel` to abort.');
        return;
      }
      await updateProduct(flow.data.product_id, { delivery_fields: fields });
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Saved \`${fields.length}\` field${fields.length === 1 ? '' : 's'}.`,
        { parse_mode: 'Markdown' },
      );
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    ...
    if (flow.type === 'edit_product_delivery_vendor') {
      if (text.trim().toLowerCase() === 'clear') {
        ...
        ctx.session.adminFlow = undefined;
        await ctx.reply('âœ… Vendor forward disabled.');
        await showProductEditor(ctx, flow.data.product_id, flow.data.page);
        return;
      }
      ...
      if (!Number.isInteger(chatId) || chatId === 0) {
        await ctx.reply(
          'âŒ Expected a non-zero integer (positive for users, negative for groups). Try again or send `/cancel`.',
        );
        return;
      }
      await updateProduct(flow.data.product_id, { delivery_vendor_chat_id: chatId });
      ctx.session.adminFlow = undefined;
      await ctx.reply(`âœ… Vendor chat saved: \`${chatId}\``, { parse_mode: 'Markdown' });
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_product_delivery_vendor_label') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(cleared ? 'âœ… Vendor label cleared.' : 'âœ… Vendor label saved.');
      await showProductEditor(ctx, flow.data.product_id, flow.data.page);
      return;
    }
    if (flow.type === 'edit_bot_tutorial_text') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Bot Tutorial text saved.');
      return;
    }
    if (flow.type === 'edit_bot_tutorial_url') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply('âœ… Bot Tutorial URL saved.');
      return;
    }
    if (flow.type === 'edit_payment_tutorial_text') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Tutorial text for method #${flow.data.method_id} saved.`,
      );
      return;
    }
    if (flow.type === 'edit_payment_tutorial_url') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Tutorial URL for method #${flow.data.method_id} saved.`,
      );
      return;
    }
    ...
    if (flow.type === 'add_payment') {
      if (flow.step === 'name') {
        ...
        ctx.session.adminFlow = undefined;
        await ctx.reply(`âœ… Payment method *${m.name}* added (id=${m.id}).`, {
          parse_mode: 'Markdown',
    ...
    if (flow.type === 'add_bybit_payment') {
      if (flow.step === 'name') {
        if (!text || text.length < 2 || text.length > 60) {
          await ctx.reply('âŒ Name must be 2â€“60 chars. Try again or `/cancel`.');
          return;
        }
        ctx.session.adminFlow = {
          type: 'add_bybit_payment',
      ...
      if (flow.step === 'bybit_id') {
        ...
        if (!/^\d{4,20}$/.test(cleaned)) {
          await ctx.reply(
            'âŒ Bybit UID / ID should be 4â€“20 digits, no spaces. Try again or `/cancel`.',
          );
          return;
        }
        ctx.session.adminFlow = {
      ...
      if (flow.step === 'bybit_name') {
        ...
        if (!trimmed || trimmed.length < 2 || trimmed.length > 64) {
          await ctx.reply(
            'âŒ Bybit Name must be 2â€“64 chars. Try again or `/cancel`.',
          );
          return;
        }
        const m = await addPaymentMethod({
    ...
    if (flow.type === 'add_binance_payment') {
      if (flow.step === 'name') {
        ...
        await ctx.reply(
          [
            'Send the *Binance Pay ID* â€” your 10-digit numeric ID (e.g. `1101801594`). The verifier rejects orders sent to any other Pay ID.',
            '',
            'You can find it in the Binance app â†’ *Pay* â†’ *Receive* â†’ it\'s the long number above your QR code.',
          ].join('\n'),
          { parse_mode: 'Markdown' },
        );
        return;
      ...
      if (flow.step === 'pay_id') {
        ...
        if (!/^\d{6,15}$/.test(cleaned)) {
          await ctx.reply(
            'âŒ Pay ID should be 6â€“15 digits, no spaces. Try again or `/cancel`.',
          );
          return;
        }
        ctx.session.adminFlow = {
        ...
        await ctx.reply(
          'Send the *Binance Pay Name* â€” the display string shown next to your Pay ID (e.g. `urweebboii`). Users see this on the deposit screen so they know they\'re paying the right account.',
          { parse_mode: 'Markdown' },
        );
        return;
      }
      if (flow.step === 'pay_name') {
        ...
        if (!trimmed || trimmed.length < 2 || trimmed.length > 64) {
          await ctx.reply(
            'âŒ Pay Name must be 2â€“64 chars. Try again or `/cancel`.',
          );
          return;
        }
        const m = await addPaymentMethod({
          name: flow.data.name,
          instructions: '(auto-verify â€” instructions are rendered by the bot)',
          min_amount: 0,
    ...
    if (flow.type === 'add_chain_payment') {
      ...
      if (flow.step === 'address') {
        ...
        if (provider === 'usdt_trc20' && !isValidTronAddress(addr)) {
          await ctx.reply(
            'âŒ Not a valid TRON address. Should start with `T` and be 34 chars. Try again or `/cancel`.',
            { parse_mode: 'Markdown' },
          );
          return;
        }
        if (provider === 'usdt_bep20' && !isValidBscAddress(addr)) {
          await ctx.reply(
            'âŒ Not a valid BSC address. Should be `0x` + 40 hex chars. Try again or `/cancel`.',
            { parse_mode: 'Markdown' },
          );
          return;
        }
        if (provider === 'usdt_ton' && !isValidTonAddress(addr)) {
          await ctx.reply(
            'âŒ Not a valid TON address. Should be `EQâ€¦` or `UQâ€¦` 48 chars. Try again or `/cancel`.',
            { parse_mode: 'Markdown' },
          );
          return;
        }
        if (provider === 'ltc' && !isValidLtcAddress(addr)) {
          await ctx.reply(
            'âŒ Not a valid Litecoin address. Should start with `L`, `M`, `3`, or `ltc1`. Try again or `/cancel`.',
            { parse_mode: 'Markdown' },
          );
          return;
        }
    ...
    if (flow.type === 'add_gift') {
      // The whole gift-create flow uses HTML â€” Markdown V1 trips on
      ...
      if (flow.step === 'code') {
        if (!/^[A-Z0-9_-]{3,40}$/i.test(text)) {
          await ctx.reply(
            'âš ï¸ Code must be 3â€“40 chars: letters, digits, <code>_</code> or <code>-</code>.',
            { parse_mode: 'HTML' },
          );
          return;
        }
        ...
        if (!Number.isFinite(amount) || amount <= 0) {
          await ctx.reply('âš ï¸ Send a positive number.');
          return;
        }
        ctx.session.adminFlow = {
          type: 'add_gift',
        ...
        if (!Number.isInteger(lim) || lim < 1) {
          await ctx.reply(
            'âš ï¸ Send a positive integer (e.g. <code>1</code>).',
            { parse_mode: 'HTML' },
          );
          return;
        }
        ...
        if (text !== '-' && text !== '') {
          ...
          if (!Number.isInteger(n) || n < 1) {
            await ctx.reply(
              'âš ï¸ Send a positive integer or <code>-</code> for unlimited.',
              { parse_mode: 'HTML' },
            );
            return;
          }
        ...
        try {
          ...
          ctx.session.adminFlow = undefined;
          await ctx.reply(
            `âœ… Gift code <code>${escapeHtml(gift.code)}</code> created â€” <b>${gift.amount} USDT</b>, ` +
              `per-user ${gift.per_user_limit}, total ${gift.max_redemptions ?? 'âˆž'}.`,
            { parse_mode: 'HTML', reply_markup: rootMenu() },
          );
        } catch (err) {
          // Distinguish the actual failure modes instead of always
          // blaming a missing migration:
          //   â€¢ 23505           â†’ code already exists (PK violation).
          //                       Keep flow at step `code` so the admin
          //                       can retype a fresh code without
          //                       restarting the wizard.
          //   â€¢ 42P01 / PGRST204 / PGRST205 â†’ table or schema-cache miss
          //                       â†’ migration 0007 isn't applied (or the
          //                       PostgREST cache is stale).
          //   â€¢ everything else â†’ surface the real DB error so the
          ...
          const detail = e?.message
            ...
            : '';
          if (e?.code === '23505') {
            // Keep the flow alive â€” drop back to the `code` step so the
            // admin doesn't lose the partial wizard input.
            ctx.session.adminFlow = { type: 'add_gift', step: 'code', data: {} };
            await ctx.reply(
              `âš ï¸ A gift code <code>${escapeHtml(flow.data.code)}</code> ` +
          ...
          if (e?.code === '42P01' || e?.code === 'PGRST204' || e?.code === 'PGRST205') {
            await ctx.reply(
              'âš ï¸ Could not create gift code â€” the bot operator must apply ' +
          ...
          await ctx.reply(
            'âš ï¸ Could not create gift code.' + detail,
            { parse_mode: 'HTML', reply_markup: rootMenu() },
          );
        }
      }
    ...
    if (flow.type === 'stats_custom_days') {
      ...
      if (!Number.isInteger(days) || days < 1 || days > 365) {
        await ctx.reply('âš ï¸ Send a whole number between 1 and 365.');
        return;
      }
      ctx.session.adminFlow = undefined;
    ...
    if (flow.type === 'set_text') {
      if (flow.step === 'key') {
        ...
        ctx.session.adminFlow = undefined;
        await ctx.reply(`âœ… Text \`${flow.data.key}\` updated.`, {
          parse_mode: 'Markdown',
          reply_markup: rootMenu(),
        });
      }
    ...
    if (flow.type === 'set_emoji') {
      ...
      // â€” Telegram surfaces it as a `custom_emoji` MessageEntity
      ...
      if (!unicode) {
        await ctx.reply('âŒ Empty value.');
        return;
      }
      ...
      const idLine = customId
        ...
        : '';
      await ctx.reply(
        `âœ… Emoji \`${flow.data.key}\` updated â†’ ${unicode}${idLine}.`,
        { parse_mode: 'Markdown', reply_markup: rootMenu() },
      );
      return;
    }
    ...
    if (flow.type === 'edit_payment_icon') {
      ...
      if (trimmed.toLowerCase() === 'clear' || trimmed === '/clear') {
        ...
        ctx.session.adminFlow = undefined;
        await ctx.reply('ðŸ§¹ Icon reset to per-provider default.');
        return;
      }
      const ce = (ctx.message.entities ?? []).find(
        (e) => e.type === 'custom_emoji' && 'custom_emoji_id' in e,
      ...
      if (!unicode) {
        await ctx.reply(
          'âŒ Couldn\'t read an emoji. Send a single emoji or `clear` to reset.',
          { parse_mode: 'Markdown' },
        );
        return;
      }
      // Sanity-check unicode length â€” a plain emoji is at most ~8
      ...
      if (!customId && unicode.length > 8) {
        await ctx.reply(
          'âš ï¸ That doesn\'t look like a single emoji.\n\n' +
      ...
      await ctx.reply(
        `âœ… Payment method #${flow.data.method_id} icon â†’ ${unicode}${idLine}.`,
        { parse_mode: 'Markdown' },
      );
      return;
    }
    ...
    if (flow.type === 'set_btnicon') {
      // Per-button icon override â†’ stored under `btnicon.<key>`,
      // separate from the shared `emoji.<key>` map. Requires a real
      // premium emoji (custom_emoji_id) â€” plain unicode can't be used
      ...
      if (!unicode || !customId || !/^\d{8,}$/.test(customId)) {
        await ctx.reply(
          'âŒ This needs a *premium* emoji. Send a premium emoji message ' +
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Button \`${flow.data.btnKey}\` icon updated â†’ ${unicode} ` +
          `(premium id \`${customId}\`).`,
        { parse_mode: 'Markdown', reply_markup: rootMenu() },
      );
      return;
    ...
    if (flow.type === 'set_color') {
      ...
      if (!flow.data.key) {
        ...
        const kb = new InlineKeyboard();
        for (const c of Object.keys(COLOR_PREFIX)) {
          kb.text(`${COLOR_PREFIX[c as ColorMode] || 'âˆ…'} ${c}`, `adm:color:set:${text}:${c}`);
        }
        backRow(kb);
    ...
    if (flow.type === 'set_color_glyph') {
      ...
      if (!(mode in COLOR_PREFIX)) {
        ctx.session.adminFlow = undefined;
        await ctx.reply('âš ï¸ Unknown color mode â€” aborted.');
        return;
      }
      const trimmed = text.trim();
      if (trimmed === '/cancel') {
      ...
      if (trimmed === '/clear') {
        ...
        ctx.session.adminFlow = undefined;
        await ctx.reply(
          `âœ… Cleared *${mode}* glyph â€” falling back to the built-in default.`,
          { parse_mode: 'Markdown' },
        );
        return;
      }
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Updated *${mode}* glyph â†’ ${glyph || '_(empty)_'}.`,
        { parse_mode: 'Markdown' },
      );
      return;
    }
    ...
    if (flow.type === 'announce') {
      ...
      if (flow.step === 'buy_label') {
        ...
        // mid-emoji on Android. Keep premium-emoji markers intact â€”
        ...
        const trimmed = text
          ...
          .slice(0, 64);
        if (!trimmed) {
          await ctx.reply('âŒ Empty label. Send the new button label or `/cancel`.');
          return;
        }
        ctx.session.adminFlow = {
          type: 'announce',
        ...
        await ctx.reply(`âœ… Label updated â†’ \`${trimmed}\``, { parse_mode: 'Markdown' });
        await showAnnounceBuyEdit(ctx);
        return;
      }
      if (flow.step === 'buy_icon') {
        ...
        if (text.trim().toLowerCase() === 'clear') {
          ...
          await ctx.reply('ðŸ—‘ Icon cleared.');
          await showAnnounceBuyEdit(ctx);
          return;
        }
        const ce = (ctx.message.entities ?? []).find(
        ...
        if (!ce) {
          await ctx.reply(
            'âŒ Send a *premium* emoji message (the bot will read its `custom_emoji_id`), or type `clear` / `/cancel`.',
            { parse_mode: 'Markdown' },
          );
          return;
        }
        ...
        if (!unicode || !customId || !/^\d{8,}$/.test(customId)) {
          await ctx.reply(
            'âŒ That emoji has no valid premium id. Send a real premium emoji, or type `clear`.',
          );
          return;
        }
        ctx.session.adminFlow = {
        ...
        await ctx.reply(
          `âœ… Icon set â†’ ${unicode} (premium id \`${customId}\`).`,
          { parse_mode: 'Markdown' },
        );
        await showAnnounceBuyEdit(ctx);
        return;
    ...
    if (flow.type === 'set_channel') {
      if (!/^https?:\/\/t\.me\//i.test(text) && !/^https?:\/\//i.test(text)) {
        await ctx.reply(
          'âŒ That doesn\'t look like a URL. Send a link like `https://t.me/yourchannel`.',
          { parse_mode: 'Markdown' },
        );
        return;
      }
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(`âœ… Channel link saved:\n\`${text}\``, {
        parse_mode: 'Markdown',
        reply_markup: rootMenu(),
      });
      return;
    ...
    if (flow.type === 'referral_adjust') {
      ...
      if (!Number.isInteger(delta) || delta === 0) {
        await ctx.reply('âŒ Send a non-zero integer, like `5` or `-3`.', {
          parse_mode: 'Markdown',
        });
        return;
      }
    ...
    if (flow.type === 'adjust_balance') {
      ...
      if (!Number.isFinite(delta)) {
        await ctx.reply('âŒ Bad number. Send e.g. `5` or `-3.5`.');
        return;
      }
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Balance adjusted by *${delta >= 0 ? '+' : ''}${delta}*. New balance: *$${newBal}*.`,
        { parse_mode: 'Markdown', reply_markup: rootMenu() },
      );
      try {
        if (delta !== 0) {
    ...
    if (flow.type === 'price_overrides_pick_user') {
      ...
      // path works regardless â€” the override system is keyed by
    ...
    if (flow.type === 'price_override_set') {
      const price = Number(text);
      if (!Number.isFinite(price) || price < 0) {
        await ctx.reply('âŒ Send a non-negative number, e.g. `9.99` or `0`.');
        return;
      }
    ...
    if (flow.type === 'price_override_bulk') {
      ...
      for (const line of lines) {
        ...
        if (!m) {
          bad.push(`â€¢ \`${line.slice(0, 40)}\` â€” bad format`);
          continue;
        }
        const product_id = Number(m[1]);
        const price = Number(m[2]);
        if (!Number.isFinite(price) || price < 0) {
          bad.push(`â€¢ \`#${product_id}\` â€” price must be â‰¥ 0`);
          continue;
        }
        const product = await getProduct(product_id);
        if (!product) {
          bad.push(`â€¢ \`#${product_id}\` â€” product not found`);
          continue;
        }
        await setUserProductPrice({
          telegram_id: target,
        ...
        ok.push(
          `â€¢ ${escapeHtml(product.name)} (\`#${product_id}\`) â†’ *$${price.toFixed(2)}*`,
        );
      }
      ctx.session.adminFlow = undefined;
      const summary = [
        `ðŸ“‹ *Bulk paste applied* for \`${target}\``,
        '',
    ...
    if (flow.type === 'set_deposit_amount') {
      const amount = Number(text);
      if (!Number.isFinite(amount) || amount <= 0) {
        await ctx.reply('âŒ Send a positive number, e.g. `5.12`.');
        return;
      }
      // numeric(14,2) â€” keep at most 2 decimals.
      const rounded = Math.floor(amount * 100) / 100;
      await setDepositAmount(flow.data.deposit_id, rounded);
      ctx.session.adminFlow = undefined;
      await ctx.reply(
        `âœ… Deposit *#${flow.data.deposit_id}* amount set to *$${rounded.toFixed(2)}*. Tap Approve to credit the user.`,
        { parse_mode: 'Markdown' },
      );
      await showDepositList(ctx);
      return;
    ...
    if (flow.type === 'promo_add' && flow.step === 'pick_user') {
      // Resolve the user â€” numeric Telegram ID always works,
      ...
      // user_product â†’ next step is product picker.
      ctx.session.adminFlow = {
        type: 'promo_add',
        step: 'pick_product',
      ...
      if (totalPages > 1) kb.text('Next â–¶', 'adm:promo:np:1');
      kb.row().text('â¬…ï¸ Cancel', 'adm:promo');
      await ctx.reply(
        [
          'âž• *New promo â€” pick a product*',
    ...
    if (flow.type === 'promo_add' && flow.step === 'min_qty') {
      ...
      if (!Number.isInteger(min_qty) || min_qty < 1) {
        await ctx.reply('âŒ Send a whole number â‰¥ 1, e.g. `10`.');
        return;
      }
      ctx.session.adminFlow = {
        type: 'promo_add',
      ...
      await ctx.reply(
        [
          'âž• *New promo â€” Step 4/4: Discount*',
          '',
          `Triggers when qty â‰¥ *${min_qty}*. Send the flat USDT discount`,
    ...
    if (flow.type === 'promo_add' && flow.step === 'discount') {
      ...
      if (!Number.isFinite(discount_amount) || discount_amount < 0) {
        await ctx.reply('âŒ Send a non-negative number, e.g. `5` or `12.5`.');
        return;
      }
      ctx.session.adminFlow = {
        type: 'promo_add',
    ...
    if (flow.type === 'promo_add' && flow.step === 'name') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(`âœ… Promo *#${created.id}* saved.`, {
        parse_mode: 'Markdown',
      });
      // Isolate the post-save card render â€” if the impact-stats query
      ...
      try {
        ...
        await ctx.reply(
          '_(Detail card render failed â€” promo is saved. Open it from /promo list.)_',
          { parse_mode: 'Markdown' },
        );
      }
      return;
    ...
    if (flow.type === 'promo_edit_qty') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(`âœ… Min qty updated to *${min_qty}*.`, { parse_mode: 'Markdown' });
    ...
    if (flow.type === 'promo_edit_discount') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(`âœ… Discount updated to *$${discount_amount.toFixed(2)}*.`, {
        parse_mode: 'Markdown',
      });
      await showPromoCard(ctx, flow.data.promo_id);
      return;
    ...
    if (flow.type === 'promo_edit_name') {
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(name ? `âœ… Name updated.` : `âœ… Name cleared.`);
      await showPromoCard(ctx, flow.data.promo_id);
      return;
    }

    if (flow.type === 'promo_exclude_add') {
      // Resolve to a numeric telegram_id â€” same logic as the new-
      ...
      ctx.session.adminFlow = undefined;
      await ctx.reply(`ðŸš« \`${telegram_id}\` is now excluded from promo #${promo_id}.`, {
        parse_mode: 'Markdown',
      });
      await showPromoExclusions(ctx, promo_id);
      return;
    ...
    ctx.session.adminFlow = undefined;
    // Surface the real DB error for promo flows â€” these have been
    ...
    if (isPromoFlow && e?.code === '42P01') {
      // undefined_table â€” migrations 0013 / 0014 didn't run.
      await ctx.reply(
        'âš ï¸ *Promo system not migrated*\n\n' +
    ...
    if (isPromoFlow && e?.code === '42703') {
      // undefined_column â€” schema partly migrated.
      await ctx.reply(
        'âš ï¸ *Promo schema is partially migrated*\n\n' +
    ...
    if (isPaymentFlow && (e?.code === '42P01' || e?.code === '42703')) {
      await ctx.reply(
        'âš ï¸ *Payment-methods schema not migrated*\n\n' +
    ...
    if (isPaymentFlow && e?.code === '23514') {
      // check_violation â€” most likely the provider CHECK constraint
      // hasn't been widened yet to accept this provider value.
      await ctx.reply(
        'âš ï¸ *Provider not allowed by the database*\n\n' +
    ...
    if (isPaymentFlow) {
      const detail = e?.message ?? String(err);
      await ctx.reply(
        'âš ï¸ *Payment-method wizard failed*\n\n' +
          `\`\`\`\n${detail.slice(0, 500)}\n\`\`\`\n` +
          (e?.hint ? `_Hint: ${escapeHtml(e.hint)}_\n` : '') +
          '\nCheck the bot logs for the full stack trace.',
        { parse_mode: 'Markdown', reply_markup: rootMenu() },
    ...
    if (flow?.type === 'edit_payment_icon') {
      // Missing table / column / schema-cache miss â†’ migration 0021
      ...
      if (e?.code === 'PGRST204' || e?.code === '42703' || e?.code === '42P01') {
        await ctx.reply(
          'âš ï¸ *Payment-methods schema not migrated*\n\n' +
            ...
            'reload the API schema (Project Settings â†’ API â†’ Restart ' +
      ...
      // Unknown DB error â€” keep the flow armed so the admin can
      ...
      const detail = e?.message ?? String(err);
      await ctx.reply(
        'âš ï¸ Couldn\'t set that as the icon.\n\n' +
          `\`\`\`\n${detail.slice(0, 500)}\n\`\`\`\n` +
          (e?.hint ? `_Hint: ${escapeHtml(e.hint)}_\n` : '') +
          '\nSend a single emoji (or a *premium* custom emoji message), ' +
    ...
    await ctx.reply('âš ï¸ Something went wrong. Cancelled.', { reply_markup: rootMenu() });
    ctx.session.adminFlow = undefined;
  }
});
...
adminBot.on('message:document', async (ctx, next) => {
  ...
  if (flow.type === 'edit_product_tutorial_file') {
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply('âœ… Tutorial document saved.');
    return;
  }
  if (flow.type === 'add_product' && flow.step === 'items') {
    ...
    if (!isTxt) {
      await ctx.reply(
        'âŒ Only `.txt` files are supported in this step. Re-upload as plain text.',
        { parse_mode: 'Markdown' },
      );
      return;
    }
    if ((doc.file_size ?? 0) > ITEMS_DOC_BYTE_CAP) {
      await ctx.reply(
        `âŒ File is too large (${doc.file_size} bytes). Cap is ${ITEMS_DOC_BYTE_CAP} bytes â€” split it and try again.`,
      );
      return;
    }
    let raw: string;
    try {
      ...
      await ctx.reply('âŒ Could not download that file. Try again in a moment.');
      return;
    }
    const payloads = parsePayloadLines(raw);
    if (payloads.length === 0) {
      await ctx.reply('âŒ The uploaded file had no non-empty lines.');
      return;
    }
    await finalizeProduct(ctx, flow.data, payloads);
    return;
  ...
  if (flow.type === 'edit_product_items') {
    ...
    if (!isTxt) {
      await ctx.reply(
        'âŒ Only `.txt` files are supported in this flow. Re-upload as plain text.',
        { parse_mode: 'Markdown' },
      );
      return;
    }
    ...
    const note =
      accepted.length < payloads.length
        ? `File capped at ${ITEMS_STAGING_CAP} â€” ${payloads.length - accepted.length} line(s) were dropped. Tap Confirm to flush, then re-upload the rest.`
  ...
  if (flow.type === 'edit_bot_tutorial_file') {
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply('âœ… Bot Tutorial document saved.');
    return;
  }
  if (flow.type === 'edit_payment_tutorial_file') {
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply(
      `âœ… Tutorial document for method #${flow.data.method_id} saved.`,
    );
    return;
  }
  if (flow.type === 'edit_payment_icon') {
    await ctx.reply(
      'âš ï¸ That\'s a document, not an emoji.\n\n' +
        'Send a single emoji (or a *premium* custom emoji message). ' +
        'Type `clear` to reset to default, or `/cancel` to abort.',
      { parse_mode: 'Markdown' },
    );
...
adminBot.on('message:photo', async (ctx, next) => {
  ...
  if (flow.type === 'edit_product_tutorial_file') {
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply('âœ… Tutorial photo saved.');
    return;
  }
  if (flow.type === 'edit_bot_tutorial_file') {
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply('âœ… Bot Tutorial photo saved.');
    return;
  }
  if (flow.type === 'edit_payment_tutorial_file') {
    await setPaymentMethodTutorialField(
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply(
      `âœ… Tutorial photo for method #${flow.data.method_id} saved.`,
    );
    return;
  }
  if (flow.type === 'edit_payment_icon') {
    ...
    await ctx.reply(
      'âš ï¸ That looks like a photo, not an emoji.\n\n' +
        'Send a single emoji (or a *premium* custom emoji message) â€” ' +
...
adminBot.on('message:sticker', async (ctx, next) => {
  ...
  if (flow.type === 'edit_payment_icon') {
    await ctx.reply(
      'âš ï¸ That\'s a sticker, not an emoji.\n\n' +
        'Send a single emoji (or a *premium* custom emoji message) â€” ' +
...
adminBot.on('message:animation', async (ctx, next) => {
  ...
  if (flow.type === 'edit_payment_icon') {
    await ctx.reply(
      'âš ï¸ That\'s an animation, not an emoji.\n\n' +
        'Send a single emoji (or a *premium* custom emoji message).',
      { parse_mode: 'Markdown' },
    );
    return;
...
adminBot.on('message:video', async (ctx, next) => {
  ...
  if (flow.type === 'edit_product_tutorial_file') {
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply('âœ… Tutorial video saved.');
    return;
  }
  if (flow.type === 'edit_bot_tutorial_file') {
    await setBotTutorialField('file_id', fileId, ctx.from.id);
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply('âœ… Bot Tutorial video saved.');
    return;
  }
  if (flow.type === 'edit_payment_tutorial_file') {
    await setPaymentMethodTutorialField(
    ...
    ctx.session.adminFlow = undefined;
    await ctx.reply(
      `âœ… Tutorial video for method #${flow.data.method_id} saved.`,
    );
    return;
  }
  if (flow.type === 'edit_payment_icon') {
    await ctx.reply(
      'âš ï¸ That\'s a video, not an emoji.\n\n' +
        'Send a single emoji (or a *premium* custom emoji message). ' +
        'Type `clear` to reset to default, or `/cancel` to abort.',
      { parse_mode: 'Markdown' },
    );
...
// video_note, audio, dice, poll, contact, location, venue, gameâ€¦)
...
// Cancelled." reply at the bottom of the text-message handler â€”
...
adminBot.on('message', async (ctx, next) => {
  ...
  await ctx.reply(
    'âš ï¸ That message isn\'t an emoji.\n\n' +
      'Send a single emoji (or a *premium* custom emoji message). ' +
      'Type `clear` to reset to default, or `/cancel` to abort.',
    { parse_mode: 'Markdown' },
  );
...
adminBot.callbackQuery('adm:cat:skip_emoji', async (ctx) => {
  ...
  cache.del('cats');
  await ctx.answerCallbackQuery();
  await ctx.editMessageText(`âœ… Category *${cat.name}* added (id=${cat.id}).`, {
    parse_mode: 'Markdown',
    reply_markup: rootMenu(),
  });
});
...
// right after the user enters the price. Yes â†’ stock=0 + unlimited=true.
// No â†’ continue to the integer stock prompt.
adminBot.callbackQuery('adm:prod:unl:yes', async (ctx) => {
  ...
  const kb = new InlineKeyboard().text('Skip', 'adm:prod:skip:warranty');
  await ctx.reply('â™¾ Stock set to *Unlimited*.\n\nSend the *warranty* text (or tap Skip).', {
    parse_mode: 'Markdown',
    reply_markup: kb,
  });
});
...
adminBot.callbackQuery('adm:prod:unl:no', async (ctx) => {
  ...
  await ctx.reply('Send the *stock* quantity (integer â‰¥ 0).', { parse_mode: 'Markdown' });
});
...
async function finalizeProduct(
  ...
  // column, so we do this as a follow-up update â€” graceful no-op
  ...
  await ctx.reply(
    [
      `âœ… Product *${product.name}* added (id=${product.id}, $${product.price}, ${stockBlurb}).`,
      items.length > 0 ? `ðŸ“¦ ${items.length} items added to the pool.` : null,
      '',
      `_Tap_ âœï¸ Edit #${product.id} _on the product list to add a premium emoji, view-note file, tutorial, or more items._`,
    ]
      .filter((s) => s !== null)
      .join('\n'),
    { parse_mode: 'Markdown', reply_markup: rootMenu() },
...
adminBot.command('settext', async (ctx) => {
  ...
  await ctx.reply(`âœ… Text \`${key}\` updated.`, { parse_mode: 'Markdown' });
});
...
adminBot.command('setcolor', async (ctx) => {
  ...
  await ctx.reply(`âœ… Color for \`${key}\` set to *${mode}*.`, { parse_mode: 'Markdown' });
});
...
adminBot.command('setemoji', async (ctx) => {
  ...
  await ctx.reply(`âœ… Emoji \`${key}\` updated.`, { parse_mode: 'Markdown' });
});
...
adminBot.command('setproductemoji', async (ctx) => {
  ...
  await ctx.reply(`âœ… Product #${id} emoji_id set.`);
});
...
adminBot.command('clearproductemoji', async (ctx) => {
  ...
  await updateProduct(id, { emoji_id: null });
  await ctx.reply(`ðŸ§¹ Product #${id} emoji_id cleared.`);
});
...
adminBot.command('setproductunlimited', async (ctx) => {
  ...
  await ctx.reply(`âœ… Product #${id} unlimited_stock = ${flag.toUpperCase()}.`);
});
...
adminBot.command('setproducttutorial', async (ctx) => {
  ...
  if (action === 'text') {
    ...
    await ctx.reply(`âœ… Product #${id} tutorial text saved.`);
    return;
  }
  if (action === 'url') {
    await updateProduct(id, { tutorial_url: value || null });
    await ctx.reply(`âœ… Product #${id} tutorial url saved.`);
    return;
  }
  if (action === 'clear') {
    await updateProduct(id, {
    ...
    await ctx.reply(`ðŸ§¹ Product #${id} tutorial wiped.`);
    return;
  }
  // file
  const reply = ctx.message?.reply_to_message;
  ...
  await ctx.reply(`âœ… Product #${id} tutorial ${file_type} saved.`);
});
...
adminBot.command('addproductitems', async (ctx) => {
  ...
  await ctx.reply(`âœ… Added ${inserted} items to product #${id}. Pool now has ${remaining} unconsumed.`);
});
...
adminBot.command('clearproductitems', async (ctx) => {
  ...
  await clearProductItems(id);
  await ctx.reply(`ðŸ§¹ Product #${id}: all items wiped.`);
});
...
adminBot.command('setbottutorial', async (ctx) => {
  ...
  if (action === 'clear') {
    ...
    await ctx.reply('ðŸ§¹ Bot Tutorial wiped.');
    return;
  }
  // file
  const reply = ctx.message?.reply_to_message;
  ...
  await ctx.reply(`âœ… Bot Tutorial ${file_type} saved.`);
});
...
adminBot.command('clearcache', async (ctx) => {
  cache.clearAll();
  await ctx.reply('ðŸ§¹ Cache cleared.');
});

adminBot.command('reload', async (ctx) => {
  await refreshSettings();
  cache.clearAll();
  await ctx.reply('ðŸ” Settings reloaded.');
});

// Diagnostic: show whether the welcome / change / delete emails will
// actually leave the bot. Useful when "no emails are arriving" â€” it
...
adminBot.command('mailerstatus', async (ctx) => {
  ...
  await ctx.reply(`ðŸ“¬ *Mailer status*\n\n\`\`\`\n${status}\n\`\`\``, {
    parse_mode: 'Markdown',
  });
});
...
adminBot.command('testemail', async (ctx) => {
  ...
  const mode = (modeRaw === 'change' || modeRaw === 'delete' ? modeRaw : 'set') as
    ...
    | 'change'
    | 'delete';
  await ctx.reply(`Sending ${mode} test email to ${target}â€¦`);
  ...
  await ctx.reply(
    ok
      ? `âœ… Sent. Check ${target}'s inbox (and spam). If nothing arrives, run /mailerstatus and check the bot logs for the Resend / SMTP error.`
      : `âŒ Send failed. Run /mailerstatus and check the logs â€” usually missing RESEND_API_KEY or unverified domain.`,
  );
});
